...
function PANEL:Init()
  ...
  --[[
  self.header.dup = createD( "DButton", self.header, 3*(self:GetWide()/5), self:GetTall(), self:GetWide()/5, 0 )
  self.header.dup:SetText( "" )
  function self.header.dup:Paint( pw, ph )
    if _pnl.uid != nil then
      paintDBButton( self, pw, ph, Color( 0, 0, 255 ), lang_string( "duplicate" ) )
    end
  end
  ]]--
  ...
  function self.listheader:Paint( pw, ph )
    ...
    surfaceText( self.textpre .. " " .. lang_string( self.text ) .. " " .. self.textpos, "SettingsNormal", pw/2, ph/2, Color( 255, 255, 255 ), 1, 1 )
  end
