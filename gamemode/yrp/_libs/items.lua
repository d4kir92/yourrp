...
function IsViewModel(ent)
	if string.find(ent:GetClass(), "viewmodel") or string.find(ent:GetModel(), "/c_") or ent:GetDBool("isviewmodel", false) or string.find(string.lower(ent:GetClass()), "c_baseflex") then
		return true
	end
	return false
end
...
if CLIENT then
	...
	function AddItemToStorage(tab)
		if tab.entity != nil then
			tab.intern_storageID = tab.entity:GetDString("storage_uid", "")
		end
