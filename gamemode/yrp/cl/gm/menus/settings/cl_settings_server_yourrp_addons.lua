...
function AddYRPAddon(parent, tab)
	...
	_add_on.icon:SetHTML(GetHTMLImage(tab.icon, ctr(100), ctr(100)))

	if tab.workshopid != "" then
		_add_on.workshop = createD("DHTML", _add_on, ctr(100), ctr(100), ctr(20), ctr(140))
		_add_on.workshop:SetHTML(GetHTMLImage("https://upload.wikimedia.org/wikipedia/commons/thumb/8/83/Steam_icon_logo.svg/120px-Steam_icon_logo.svg.png", ctr(100), ctr(100)))
		_add_on.workshopbutton = createD("DButton", _add_on.workshop, ctr(100), ctr(100), 0, 0)
	...
	if tab.discord != "" then
		_add_on.discord = createD("DHTML", _add_on, ctr(100), ctr(100), ctr(20), ctr(260))
		_add_on.discord:SetHTML(GetHTMLImage("https://discordapp.com/assets/f8389ca1a741a115313bede9ac02e2c0.svg", ctr(100), ctr(100)))
		_add_on.discordbutton = createD("DButton", _add_on.discord, ctr(100), ctr(100), 0, 0)
	...
	if tab.settings != "" then
		_add_on.settings = createD("DButton", _add_on, ctr(100), ctr(100), ctr(20), ctr(380))
	...
	if tab.name != "" then
		...
		_add_on.name:SetText(YRP.lang_string("ID_name") .. ": " .. tab.name)
	end
	...
	if tab.author != "" then
		...
		_add_on.author:SetText(YRP.lang_string("ID_s_author") .. ": " .. tab.author)
	end
	...
	if tab.description != "" then
		...
		_add_on.description:SetText(YRP.lang_string("ID_description") .. ":\n" .. tab.description)
	end
...
net.Receive("Connect_Settings_YourRP_Addons", function(len)
	if pa(settingsWindow) then
		...
		local yrp_addons = createD("DPanelList", PARENT, ctr(1600), ScrH() - ctr(140), ctr(20), ctr(20))
