...
function OpenPlayerOptions(ply)
	...
	if lp:HasAccess() then
		...
		_menu:AddOption("Language: " .. ply:GetLanguage(), "icon16/map.png")
		_menu:AddSpacer()
...
function drawGroup(id, name, color, icon, iter)
	...
	if hasGroupPlayers(id) then
		...
		function _tmpHeader:Paint(pw, ph)
			...
			--draw.SimpleTextOutlined(YRP.lang_string("LID_level"), "sef", ctr(10), ph/2, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))

			local _w = ctr(128+16)

			local str = YRP.lang_string("LID_name")
			if ply:GetNWBool("bool_yrp_scoreboard_show_usergroup", false) then
				str = YRP.lang_string("LID_name") .. "/" .. YRP.lang_string("LID_usergroup")
			end
			...
			str = ""
			if ply:GetNWBool("bool_yrp_scoreboard_show_rolename", false) then
				str = YRP.lang_string("LID_role")
			end
			if ply:GetNWBool("bool_yrp_scoreboard_show_groupname", false) then
				if str != "" then
					str = str .. "/"
				end
				str = str .. YRP.lang_string("LID_group")
			end
			if ply:GetNWBool("bool_yrp_scoreboard_show_rolename", false) or ply:GetNWBool("bool_yrp_scoreboard_show_groupname", false) then
				_w = _w + ctr(600)
				draw.SimpleTextOutlined(str, "sef", _w, ph/2, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
			...
			str = ""
			if ply:GetNWBool("bool_yrp_scoreboard_show_frags", false) then
				str = YRP.lang_string("LID_frags")
			end
			if ply:GetNWBool("bool_yrp_scoreboard_show_deaths", false) then
				if str != "" then
					str = str .. "/"
				end
				str = str .. YRP.lang_string("LID_deaths")
			end
			if ply:GetNWBool("bool_yrp_scoreboard_show_frags", false) or ply:GetNWBool("bool_yrp_scoreboard_show_deaths", false) then
				_w = _w + ctr(600)
				draw.SimpleTextOutlined(str, "sef", _w, ph/2, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
			...
			if ply:GetNWBool("bool_yrp_scoreboard_show_playtime", false) then
				_w = _w + ctr(500)
				draw.SimpleTextOutlined(YRP.lang_string("LID_playtime"), "sef", _w, ph/2, Color(255, 255, 255, 255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
			end

			if ply:GetNWBool("bool_yrp_scoreboard_show_operating_system", false) then
				draw.SimpleTextOutlined(YRP.lang_string("LID_os"), "sef", pw - ctr(150), ph/2, Color(255, 255, 255, 255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
			end

			draw.SimpleTextOutlined(YRP.lang_string("LID_ping"), "sef", pw - ctr(20), ph/2, Color(255, 255, 255, 255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
			--draw.SimpleTextOutlined(YRP.lang_string("LID_mute"), "sef", pw - ctr(100), ph/2, Color(255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
		end
		elePos.y = elePos.y + 50
		drawGroupPlayers(id)
	end
...
function drawRest()
	...
	if count > 0 then
		...
		function _tmpHeader:Paint(pw, ph)
			draw.RoundedBox(0, 0, 0, pw, ph, self.color)
			draw.SimpleTextOutlined(YRP.lang_string("LID_unassigned"), "sef", ctr(10), ctr(25), Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
		end
	...
	if LocalPlayer():GetNWBool("bool_yrp_scoreboard", false) then
		...
		if _SBFrame == nil then
			...
			function _SBFrame:Paint(pw, ph)
				...
				draw.SimpleTextOutlined(YRP.lang_string("LID_map") .. ": " .. GetNiceMapName(), "ScoreBoardNormal", pw - ctr(256 + 20), ctr(75), Color(255, 255, 255, 255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
				draw.SimpleTextOutlined(YRP.lang_string("LID_players") .. ": " .. #player.GetAll() .. "/" .. game.MaxPlayers(), "ScoreBoardNormal", pw - ctr(256 + 20), ctr(125), Color(255, 255, 255, 255), TEXT_ALIGN_RIGHT, TEXT_ALIGN_CENTER, 1, Color(0, 0, 0))
...
function GM:ScoreboardShow()
	if LocalPlayer():GetNWString("text_yrp_scoreboard_style", "") == "advanced" then
		scoreboard:show_sb()
	else
		OpenSBS()
	end
