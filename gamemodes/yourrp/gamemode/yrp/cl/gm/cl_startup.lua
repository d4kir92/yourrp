...
function openSingleSelector(tab, closeF, web)
	...
	function nextB:Paint(pw, ph)
		draw.RoundedBox(0, 0, 0, pw, ph, Color( 255, 255, 255, 255 ) )
		draw.SimpleTextOutlined(YRP.lang_string( "LID_next" ), "Y_24_500", pw / 2, ph / 2, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color( 0, 0, 0, 255 ) )
	end
	...
	function prevB:Paint(pw, ph)
		draw.RoundedBox(0, 0, 0, pw, ph, Color( 255, 255, 255, 255 ) )
		draw.SimpleTextOutlined(YRP.lang_string( "LID_prev" ), "Y_24_500", pw / 2, ph / 2, Color( 255, 255, 255, 255 ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color( 0, 0, 0, 255 ) )
	end
...
function YRPDrawNamePlates()
	...
	if IsValid( LocalPlayer() ) and player.GetAll() then
		for i, ply in pairs( player.GetAll() ) do -- NAMEPLATES
		...
		for i, ply in SortedPairsByMemberValue( player.GetAll(), "ply_distance", true ) do
			...
			if GetGlobalYRPBool( "bool_server_debug_voice", false) and LocalPlayer():GetPos():Distance(ply:GetPos() ) < 1000 then
				...
				if ply == LocalPlayer() then
					...
					if LocalPlayer():GetPos():Distance(ply:GetPos() ) < GetGlobalYRPInt( "int_voice_max_range", 1) then
						col = Color( 100, 255, 100, 120)
					end
				end
			...
			if LocalPlayer():GetPos():Distance(ply:GetPos() ) < renderdist and ply:Alive() and !ply:InVehicle() then
				...
				if GetGlobalYRPBool( "bool_tag_on_head", false) then
					if GetGlobalYRPBool( "bool_tag_on_head_voice", false) and ply:GetYRPBool( "yrp_speaking", false) then
						...
						if ply:VoiceVolume() > 0.50 then -- Schreien
							voicecolor = Color( 255, 100, 100, plyvol )
						elseif ply:VoiceVolume() > 0.20 then -- Laut
							voicecolor = Color( 220, 220, 80, plyvol )
						elseif ply:VoiceVolume() > 0.08 then -- Normal
							voicecolor = Color( 80, 255, 80, plyvol )
						elseif ply:VoiceVolume() > 0.04 then -- Flüstern
				...
				if ply:GetYRPBool( "tag_ug", false) or (GetGlobalYRPBool( "show_tags", false) and ply:GetMoveType() == MOVETYPE_NOCLIP and !ply:InVehicle() ) and color.a > 10 then
					local ugcolor = ply:GetUserGroupColor()
					ugcolor.a = color.a
			...
			if !string.find(ele, "logo", 1, true) and !string.find(ele, "background", 1, true) then
				if string.find(ele, "box", 1, true) then
					...
					elseif ele == "securitylevel" then
						text = YRP.lang_string( "LID_" .. ele) .. " " .. ply:SecurityLevel()
...
hook.Add( "Think", "openDeathScreen", function(len)
	...
	if !YRP_LogOut and LocalPlayer():LoadedGamemode() and !LocalPlayer():Alive() and !vgui.CursorVisible() and dsd < CurTime() and LocalPlayer():CharID() > 0 and !ds and GetGlobalYRPBool( "bool_deathscreen", false) and !customdeathscreen then
		...
		function win.respawn:Paint(pw, ph)
			if !LocalPlayer():GetYRPBool( "yrp_chararchived", false) then
				local tab = {}
...
if pa(yrp_loading_screen) then
	...
	yrp_loading_screen.blur.tmaxmax = 90
	...
	function yrp_loading_screen.blur:Paint(pw, ph)
		...
		if YRPReceivedServerData() then
			loading_cur = 100
		elseif YRPReceivedStartData() then
			loading_cur = 50
		end
		...
		if yrp_loading_screen.bg then
			if yrp_loading_screen.bg.url != GetGlobalYRPString( "text_loading_background" ) then
				yrp_loading_screen.bg.url = GetGlobalYRPString( "text_loading_background" )
		...
		if self.t >= 0 then
			...
			if GetBranch() == "32Bit" and self.t >= 30 then
				draw.SimpleText( YRP.lang_string( "LID_youareusing32bit" ), "Y_" .. 30 .. "_700", pw / 2, ScrH() * WARNING_PY - 30, TextColor(YRPCPP() ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
			end
			draw.SimpleText( text, "Y_" .. 30 .. "_700", pw / 2, ScrH() * WARNING_PY, TextColor(YRPCPP() ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER )
		end

		-- BAR --
		local text = ""
		if !YRPReceivedStartData() then
			text = YRP.lang_string( "LID_sendclientinfo" )
		elseif !YRPReceivedServerData() then
			text = YRP.lang_string( "LID_receiveserverdata" )
		end
		if strEmpty( text ) then
			text = YRP.lang_string( "LID_done" ) .. " " .. math.ceil(loading_cur_old) / max * 100 .. "%"
		else
			text = YRP.lang_string( "LID_loading" ) .. " (" .. text .. ") " .. math.ceil(loading_cur_old) / max * 100 .. "%"
		end
		if GetGlobalYRPString( "text_loading_design" ) == "Center" then
			...
			draw.SimpleText( text, "Y_" .. BAR_FONT_SIZE .. "_700", pw / 2, SCREEN_CENTER_Y - PANEL_H / 2 + BAR_SPACE + BAR_H / 2, TextColor(YRPCPP() ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		elseif GetGlobalYRPString( "text_loading_design" ) == "BottomRight" then
			...
			draw.SimpleText( text, "Y_" .. BAR_FONT_SIZE .. "_700", ScrW() - BAR_W / 2 - BAR_SPACE, ScrH() - BAR_H / 2 - BAR_SPACE, TextColor(YRPCPP() ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		else
			...
			draw.SimpleText( text, "Y_" .. BAR_FONT_SIZE .. "_700", pw / 2, ScrH() - BAR_H / 2 - BAR_SPACE, TextColor(YRPCPP() ), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
		end
		...
		-- TIME
		local bl1 = net.BytesLeft()
		local bw1 = net.BytesWritten()
		local text = YRP.lang_string( "LID_time" ) .. ": " .. self.t .. "/" .. self.tmax .. "    PacketLoss: " .. lply:PacketLoss()
		if bl1 == nil then
			bl1 = 0
		end
		if bw1 == nil then
		...
		text = text .. "    BytesLeft: " .. bl1
		text = text .. "    BytesWritten: " .. bw1
		...
		window.tabs:AddOption( "LID_prisoners", function(parent)
			...
			function addButton:DoClick()
				...
				function content:Paint(pw, ph)
					draw.SimpleTextOutlined(YRP.lang_string( "LID_player" ), "Y_24_500", YRP.ctr(10), YRP.ctr(50), Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM, 1, Color( 0, 0, 0, 255 ) )
					draw.SimpleTextOutlined(YRP.lang_string( "LID_cell" ), "Y_24_500", YRP.ctr(10), YRP.ctr(150), Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM, 1, Color( 0, 0, 0, 255 ) )
					draw.SimpleTextOutlined(YRP.lang_string( "LID_note" ), "Y_24_500", YRP.ctr(10), YRP.ctr(250), Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM, 1, Color( 0, 0, 0, 255 ) )
					draw.SimpleTextOutlined(YRP.lang_string( "LID_timeinsec" ), "Y_24_500", YRP.ctr(10), YRP.ctr(350), Color( 255, 255, 255, 255 ), TEXT_ALIGN_LEFT, TEXT_ALIGN_BOTTOM, 1, Color( 0, 0, 0, 255 ) )
				end
		...
		window.tabs:AddOption( "LID_records", function(parent)
			...
			function btnVerwarnung:Paint(pw, ph)
				if IsValid(plist) and plist.ply and IsValid( plist.ply ) and plist.ply:IsPlayer() then
					...
					btnVerwarnung:SetText(YRP.lang_string( "LID_warnings" ) .. ": " .. plist.ply:GetYRPInt( "int_warnings", -1) )
				end
			end
			...
			function btnVerstoesse:Paint( pw, ph )
				if IsValid(plist) and plist.ply and IsValid( plist.ply ) and plist.ply:IsPlayer() then
					hook.Run( "YLabelPaint", self, pw, ph)
					btnVerstoesse:SetText(YRP.lang_string( "LID_violations" ) .. ": " .. plist.ply:GetYRPInt( "int_violations", -1) )
				end
			end
			...
			function btnArrests:Paint(pw, ph)
				if IsValid(plist) and plist.ply and IsValid( plist.ply ) and plist.ply:IsPlayer() then
					hook.Run( "YLabelPaint", self, pw, ph)
					btnArrests:SetText(YRP.lang_string( "LID_arrests" ) .. ": " .. plist.ply:GetYRPInt( "int_arrests", -1) )
				end
			end
