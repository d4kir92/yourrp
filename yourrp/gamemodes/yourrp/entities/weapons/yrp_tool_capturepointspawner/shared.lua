...
SWEP.Purpose = ""
SWEP.Instructions = "Leftclick - Create capturepoint\nRightclick - Remove capturepoint"

SWEP.Category = "[YourRP] Admin"

SWEP.PrintName = "Tool capturepoint Spawner"
SWEP.Language = "en"
SWEP.LanguageString = "Tool capturepoint Spawner"
...
function SWEP:Reload()
	local pos = ""
	for i, v in pairs(GetGlobalDTable("yrp_capturepoints")) do
		pos = v.pos
	end
	if pos != "" then
...
if SERVER then
	util.AddNetworkString("yrp_capturepoints_options")
end
		...
		if ply:KeyDown(IN_USE) and !keydown then
			...
			for i, v in pairs(GetGlobalDTable("yrp_capturepoints")) do
				...
				if p:Distance(pos) < size * 2 then
					...
					local stab = SQL_SELECT("yrp_" .. GetMapNameDB(), "*", "type = 'capturepoint' AND uniqueID = '" .. v.uniqueID .. "'")
					if wk(stab) then
						stab = stab[1]
						net.Start("yrp_capturepoints_options")
							net.WriteTable(stab)
						net.Send(ply)
					end
				end
...
if CLIENT then
	net.Receive("yrp_capturepoints_options", function()
	...
	if self.pdelay < CurTime() then
		...
		if CLIENT then
			...
			net.Start("dbInsertIntoMap")
				...
				local tmpString = "'" .. tonumber(tmpPos[1]) .. "," .. tonumber(tmpPos[2]) .. "," .. tonumber(tmpPos[3] + 4) .. "', '" .. tonumber(tmpAng[1]) .. "," .. tonumber(tmpAng[2]) .. "," .. tonumber(tmpAng[3]) .. "', 'capturepoint'"
		...
		if SERVER then
			...
			YRP.msg("db", "Added capturepoint")

			UpdatecapturepointTable()
		end
	end
end
...
function SWEP:SecondaryAttack()
	if SERVER then
		...
		for i, v in pairs(GetGlobalDTable("yrp_capturepoints")) do
			local p = StringToVector(v.pos)
			if p:Distance(pos) < size * 2 then
				SQL_DELETE_FROM("yrp_" .. GetMapNameDB(), "type = 'capturepoint' AND uniqueID = '" .. v.uniqueID .. "'")
...
if CLIENT then
	...
	local delay = CurTime()
	hook.Add("PostDrawTranslucentRenderables", "yrp_draw_capturepoint", function()
		if LocalPlayer():GetActiveWeapon():IsValid() and LocalPlayer():GetActiveWeapon():GetClass() == "yrp_tool_capturepointspawner" then
