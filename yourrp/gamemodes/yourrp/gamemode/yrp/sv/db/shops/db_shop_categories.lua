...
SQL_ADD_COLUMN(DATABASE_NAME, "shopID", "INT DEFAULT -1")
...
function send_categories(ply, uid)
	local _cats = SQL_SELECT(DATABASE_NAME, "*", "shopID = " .. uid)
	local _nw = _cats
	if _nw == nil then
		_nw = {}
	end
...
net.Receive("category_add", function(len, ply)
	local _shopid = net.ReadString()
	local _new = SQL_INSERT_INTO(DATABASE_NAME, "shopID", _shopid)
...
net.Receive("yrp_shop_get_categories", function(len, ply)
	local _uid = net.ReadString()
	local _cats = SQL_SELECT(DATABASE_NAME, "*", "shopID = '" .. _uid .. "'")
	local _nw = {}
	if _cats != nil then
		_nw = _cats
	end
