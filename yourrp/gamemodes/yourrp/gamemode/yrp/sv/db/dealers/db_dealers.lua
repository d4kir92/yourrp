...
SQL_ADD_COLUMN(DATABASE_NAME, "name", "TEXT DEFAULT 'Unnamed dealer'")
SQL_ADD_COLUMN(DATABASE_NAME, "tabs", "TEXT DEFAULT ''")
SQL_ADD_COLUMN(DATABASE_NAME, "WorldModel", "TEXT DEFAULT 'models/player/skeleton.mdl'")
SQL_ADD_COLUMN(DATABASE_NAME, "map", "TEXT DEFAULT 'gm_construct'")
SQL_ADD_COLUMN(DATABASE_NAME, "storagepoints", "TEXT DEFAULT ''")

--db_drop_table(DATABASE_NAME)
--db_is_empty(DATABASE_NAME)

if SQL_SELECT(DATABASE_NAME, "*", "uniqueID = 1") == nil then
	local _global_shop = SQL_INSERT_INTO(DATABASE_NAME, "name, uniqueID", "'LID_buymenu', 1")
end

local _minus = SQL_SELECT(DATABASE_NAME, "*", "uniqueID = '-1'")
if _minus != nil then
	SQL_DELETE_FROM(DATABASE_NAME, "uniqueID = '-1'")
end
...
function dealer_rem(uid)
	uid = tonumber(uid)
	local _del = SQL_DELETE_FROM(DATABASE_NAME, "uniqueID = '" .. uid .. "'")
end
...
function dealer_add(ply)
	...
	local _insert = SQL_INSERT_INTO(DATABASE_NAME, "name, map", "'" .. _uid .. "', '" .. GetMapNameDB() .. "'")
	local _db_sel = SQL_SELECT(DATABASE_NAME, "uniqueID", "name = '" .. _uid .. "'")

	if _db_sel != nil then
		_db_sel = _db_sel[1]
		local _db_upd = SQL_UPDATE(DATABASE_NAME, {["name"] = "Unnamed Dealer"}, "uniqueID = " .. _db_sel.uniqueID)
...
net.Receive("dealer_add_tab", function(len, ply)
	...
	local _dealer = SQL_SELECT(DATABASE_NAME, "*", "uniqueID = " .. _dealer_uid)
	...
	if _dealer != nil then
		...
		local _up = SQL_UPDATE(DATABASE_NAME, {["tabs"] = _tabs}, "uniqueID = " .. _dealer_uid)
	end
end)
...
net.Receive("dealer_rem_tab", function(len, ply)
	...
	if _dealer != nil then
		...
		SQL_UPDATE(DATABASE_NAME, {["tabs"] = _dealer.tabs}, "uniqueID = " .. _dealer_uid)
	end
end)
...
net.Receive("dealer_edit_name", function(len, ply)
	...
	local _dealer = SQL_UPDATE(DATABASE_NAME, {["name"] = _dealer_new_name}, "uniqueID = " .. _dealer_uid)
...
net.Receive("dealer_edit_worldmodel", function(len, ply)
	...
	local _dealer = SQL_UPDATE(DATABASE_NAME, {["WorldModel"] = _dealer_new_wm}, "uniqueID = " .. _dealer_uid)
	for i, npc in pairs(ents.GetAll()) do
		if npc:GetNW2String("dealerID", "FAILED") == tostring(_dealer_uid) then
...
net.Receive("dealer_edit_storagepoints", function(len, ply)
	...
	local _dealer = SQL_UPDATE(DATABASE_NAME, {["storagepoints"] = _dealer_storagepoints}, "uniqueID = " .. _dealer_uid)
end)
