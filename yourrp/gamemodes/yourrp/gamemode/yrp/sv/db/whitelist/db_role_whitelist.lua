...
net.Receive("whitelistPlayer", function(len, ply)
	...
	if ply:GetNW2Bool("bool_whitelist") then
		...
		if wk(DBRole) then
			...
			YRP_SQL_INSERT_INTO("yrp_role_whitelist", "SteamID, nick, groupID, roleID, date, status, name", "'" .. _SteamID .. "', '" .. _nick .. "', " .. _groupID .. ", " .. roleID .. ", '" .. dat .. "', '" .. status .. "', '" .. name .. "'")
...
net.Receive("whitelistPlayerGroup", function(len, ply)
	if ply:HasAccess() then
		...
		local status = "Manually by " .. ply:SteamName()
		local name = target:SteamName()
		YRP_SQL_INSERT_INTO("yrp_role_whitelist", "SteamID, nick, groupID, date, status, name", "'" .. _SteamID .. "', '" .. _nick .. "', " .. groupID .. ", '" .. dat .. "', '" .. status .. "', '" .. name .. "'")
...
net.Receive("whitelistPlayerAll", function(len, ply)
	if ply:HasAccess() then
		...
		local status = "Manually by " .. ply:SteamName()
		local name = target:SteamName()
		YRP_SQL_INSERT_INTO("yrp_role_whitelist", "SteamID, nick, roleID, groupID, date, status, name", "'" .. _SteamID .. "', '" .. _nick .. "', " .. "-1" .. ", " .. "-1" .. ", '" .. dat .. "', '" .. status .. "', '" .. name .. "'")
