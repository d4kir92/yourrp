	...
	if GetGlobalBool( "bool_yrp_showowner", true) then
		if eyeTrace.Entity:GetOwner() != nil and eyeTrace.Entity:GetOwner() != NULL then
			...
			elseif !strEmpty(eyeTrace.Entity:GetYRPString( "ownerRPName", "" ) ) or !strEmpty(eyeTrace.Entity:GetYRPString( "ownerGroup", "" ) ) then
				...
				draw.SimpleText(YRP.lang_string( "LID_owner" ) .. ": " ..	eyeTrace.Entity:GetYRPString( "ownerRPName", "" ) .. groupname, "Y_24_500", ScrW() / 2, ScrH2() + YRP.ctr(750), Color( 255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color( 0, 0, 0) )
			end
		end
	end
end
		...
		if ea(ent) then
			...
			elseif ent:IsNPC() then
				if ent:IsDealer() then
					draw.SimpleText(ent:GetYRPString( "name", "" ), "Y_24_500", ScrW() / 2, ScrH2() + YRP.ctr(150), Color( 255, 255, 255, 255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, Color( 0, 0, 0) )
					local key = {}
