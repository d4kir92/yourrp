...
hook.Add("PlayerDisconnected", "YRPPlayerDisconnected", function(ply)
	...
	for i, ent in pairs(ents.GetAll()) do
		if IsValid(ent) then
			if ent.PermaProps or ent.PermaPropsID ~= nil or ent.PermaProps_ID ~= nil then -- if perma propped => ignore
				continue
			end

			if ent:GetOwner() == ply or ent:GetRPOwner() == ply then ent:Remove() end
		end
	end

	if IsValid(ply) then
		local ragdoll = ply:GetRagdollEntity()
		if IsValid(ragdoll) and ragdoll.index == ply:GetYRPInt("ent_ragdollindex") then ragdoll:Remove() end
	end
end)
...
hook.Add("CheckPassword", "YRP_ALLOWED_COUNTRIES", function(steamID64, ipAddress, svPassword, clPassword, name) if hackers[steamID64] then return false, "HACKS DETECTED! VAC BAN INCOMING" end end)
hook.Add("PlayerAuthed", "yrp_PlayerAuthed", function(ply, steamid, uniqueid)
	...
	if IsValid(ply) and ply.KillSilent then ply:KillSilent() end
...
function YRPPlyUpdateStorage(ply)
	if not IsValid(ply) then return false end
	local chaTab = ply:YRPGetCharacterTable()
	if IsNotNilAndNotFalse(chaTab) and not IsVoidCharEnabled() then ply:SetYRPString("storage", chaTab.storage) end
	return true
end
		...
		if ply:HasCharacterSelected() then
			...
			if IsNotNilAndNotFalse(plyT) then
				...
				if IsNotNilAndNotFalse(_rol_tab) then
					...
					local chatab = ply:YRPGetCharacterTable()
					if IsNotNilAndNotFalse(chatab) then CheckIfRoleExists(ply, chatab.roleID) end
					ply:KillSilent()
				end
...
hook.Add("PlayerLoadout", "yrp_PlayerLoadout", function(ply)
	...
	timer.Simple(0, function() if IsValid(ply) then YRPPlayerLoadout(ply) end end)
	return true
end)
		...
		timer.Simple(0.2, function()
			if IsValid(ply) then
				pcall(function(pl)
					...
					if type(err) == "string" then YRP:msg("note", "ERROR [PlayerSpawn]: " .. tostring(err) .. " code: " .. tostring(code)) end
					RSPLY = nil
				end, ply)
			end
		end)
...
function AddStar(ply)
	if IsValid(ply) then
		...
		if rand <= chance then
			...
			if ply:GetYRPInt("yrp_stars", 0) > 5 then ply:SetYRPInt("yrp_stars", 5) end
		end
	end
end
...
hook.Add("PlayerDeath", "yrp_stars_playerdeath", function(victim, inflictor, attacker)
	if attacker:IsPlayer() then AddStar(attacker) end
...
hook.Add("OnNPCKilled", "yrp_stars_onnpckilled", function(npc, attacker, inflictor) AddStar(attacker) end)
...
function PLAYER:YRPCreateRagdoll()
	if PLAYER.OldCreateRagdoll ~= PLAYER.CreateRagdoll then self:PrintMessage(HUD_PRINTCENTER, "[REVIVE] There is another REVIVE SYSTEM INSTALLED") end
	local ply = self
	...
	if IsValid(rd) and ply:GetModel() ~= nil then
		...
		for id = 0, 19 do
			...
			if val then rd:SetBodygroup(id, val) end
		end
...
function GM:DoPlayerDeath(ply, attacker, dmginfo)
	...
	if PLAYER.OldCreateRagdoll ~= PLAYER.CreateRagdoll then ply:CreateRagdoll() end
	ply:AddDeaths(1)
...
hook.Add("DoPlayerDeath", "yrp_player_spawn_DoPlayerDeath", function(ply, attacker, dmg)
	if attacker.SteamID and ply.SteamID then
		...
		if time and damage and steamId and attackerSteamId then YRP_SQL_INSERT_INTO("yrp_logs", "string_timestamp, string_typ, string_source_steamid, string_target_steamid, string_value", "'" .. time .. "' ,'LID_kills', '" .. attackerSteamId .. "', '" .. steamId .. "', '" .. damage .. "'") end
	end
...
function GM:PlayerDeathThink(pl)
	...
	if YRPDeathKeys(pl) then
		if pl:IsBot() then pl:Spawn() end
		if GetGlobalYRPBool("bool_deathscreen", false) == false then pl:Spawn() end
	end
end
	...
	if IsInsideSafezone(ent) or (ent:IsPlayer() and (ent:HasGodMode() or ent:GetYRPBool("godmode", false))) then
		...
		if IsValid(attacker) and IsInsideSafezone(attacker) then
			...
			elseif ent:IsPlayer() then
				if GetGlobalYRPBool("bool_antipropkill", true) and IsValid(dmginfo:GetAttacker()) and dmginfo:GetAttacker():GetClass() == "prop_physics" then dmginfo:ScaleDamage(0) end
...
function StartCombat(ply)
	if ply:IsValid() and ply:IsPlayer() then
		...
		local steamid = ply:YRPSteamID()
		if timer.Exists(steamid .. " outOfCombat") then timer.Remove(steamid .. " outOfCombat") end
	...
	if ply and ply.IsFullyAuthenticated and ply:IsFullyAuthenticated() then
		if IsInsideSafezone(ply) or ply:HasGodMode() or ply:GetYRPBool("godmode", false) then
			...
			if IsValid(attacker) and IsInsideSafezone(attacker) then
				...
				if IsValid(dmginfo:GetAttacker()) and dmginfo:GetAttacker() ~= ply then StartCombat(ply) end
				SlowThink(ply)
				if GetGlobalYRPBool("bool_antipropkill", true) and IsValid(dmginfo:GetAttacker()) and dmginfo:GetAttacker():GetClass() == "prop_physics" then dmginfo:ScaleDamage(0) end
						...
						if IsBonefracturingEnabled() then
							...
							if _break <= GetBrokeChanceArms() then
								if hitgroup == HITGROUP_LEFTARM then
									...
									if not ply:HasWeapon("yrp_unarmed") then ply:Give("yrp_unarmed") end
									ply:SelectWeapon("yrp_unarmed")
								elseif hitgroup == HITGROUP_RIGHTARM then
									ply:SetYRPBool("broken_arm_right", true)
									if not ply:HasWeapon("yrp_unarmed") then ply:Give("yrp_unarmed", true) end
									ply:SelectWeapon("yrp_unarmed")
								end
							end
						end
...
net.Receive("nws_yrp_voice_start", function(len, ply) ply:SetYRPBool("yrp_speaking", true) end)
YRP:AddNetworkString("nws_yrp_voice_end")
net.Receive("nws_yrp_voice_end", function(len, ply) ply:SetYRPBool("yrp_speaking", false) end)
YRP:AddNetworkString("nws_yrp_mute_voice")
net.Receive("nws_yrp_mute_voice", function(len, ply) ply:SetYRPBool("mute_voice", not ply:GetYRPBool("mute_voice", false)) end)
YRP:AddNetworkString("nws_yrp_voice_range_up")
net.Receive("nws_yrp_voice_range_up", function(len, ply) ply:SetYRPInt("voice_range", math.Clamp(ply:GetYRPInt("voice_range", 2) + 1, 0, 4)) end)
YRP:AddNetworkString("nws_yrp_voice_range_dn")
net.Receive("nws_yrp_voice_range_dn", function(len, ply) ply:SetYRPInt("voice_range", math.Clamp(ply:GetYRPInt("voice_range", 2) - 1, 0, 4)) end)
-- VOICE CHANNELS
...
function GenerateVoiceTable()
	...
	if IsNotNilAndNotFalse(channels) then
		for i, channel in pairs(channels) do
			...
			-- ACTIVE
			local augs = {}
			if channel.string_active_usergroups then augs = string.Explode(",", channel.string_active_usergroups) end
			yrp_voice_channels[tonumber(channel.uniqueID)]["string_active_usergroups"] = {}
			for _, ug in pairs(augs) do
				if not strEmpty(ug) then yrp_voice_channels[tonumber(channel.uniqueID)]["string_active_usergroups"][ug] = true end
			end

			local agrps = {}
			if channel.string_active_groups then agrps = string.Explode(",", channel.string_active_groups) end
			yrp_voice_channels[tonumber(channel.uniqueID)]["string_active_groups"] = {}
			for _, grp in pairs(agrps) do
				if not strEmpty(grp) then yrp_voice_channels[tonumber(channel.uniqueID)]["string_active_groups"][tonumber(grp)] = true end
			end

			local arols = {}
			if channel.string_active_roles then arols = string.Explode(",", channel.string_active_roles) end
			yrp_voice_channels[tonumber(channel.uniqueID)]["string_active_roles"] = {}
			for _, rol in pairs(arols) do
				if not strEmpty(rol) then yrp_voice_channels[tonumber(channel.uniqueID)]["string_active_roles"][tonumber(rol)] = true end
			end

			-- PASSIVE
			local pugs = {}
			if channel.string_passive_usergroups then pugs = string.Explode(",", channel.string_passive_usergroups) end
			yrp_voice_channels[tonumber(channel.uniqueID)]["string_passive_usergroups"] = {}
			for _, ug in pairs(pugs) do
				if not strEmpty(ug) then yrp_voice_channels[tonumber(channel.uniqueID)]["string_passive_usergroups"][ug] = true end
			end

			local pgrps = {}
			if channel.string_passive_groups then pgrps = string.Explode(",", channel.string_passive_groups) end
			yrp_voice_channels[tonumber(channel.uniqueID)]["string_passive_groups"] = {}
			for _, grp in pairs(pgrps) do
				if not strEmpty(grp) then yrp_voice_channels[tonumber(channel.uniqueID)]["string_passive_groups"][tonumber(grp)] = true end
			end

			local prols = {}
			if channel.string_passive_roles then prols = string.Explode(",", channel.string_passive_roles) end
			yrp_voice_channels[tonumber(channel.uniqueID)]["string_passive_roles"] = {}
			for _, rol in pairs(prols) do
				if not strEmpty(rol) then yrp_voice_channels[tonumber(channel.uniqueID)]["string_passive_roles"][tonumber(rol)] = true end
			end
		end
	else
		yrp_voice_channels = {}
...
hook.Add("YRP_SQLDBREADY_COMMUNICATION_DB", "yrp_voice_channels", function()
	...
	if YRP_SQL_SELECT(DATABASE_NAME, "*") == nil then YRP_SQL_INSERT_INTO(DATABASE_NAME, "string_name, int_hear, string_mode, string_active_usergroups, string_passive_usergroups", "'DEFAULT', '1', '0', 'superadmin, admin, user', 'superadmin, admin, user'") end
end)

hook.Add("YRP_SQLDBREADY_COMMUNICATION", "yrp_voice_channels", function() GenerateVoiceTable() end)
...
function YRPCountActiveChannels(ply)
	...
	for i, channel in SortedPairsByMemberValue(GetGlobalYRPTable("yrp_voice_channels", {}), "int_position", false) do
		if IsActiveInChannel(ply, channel.uniqueID, true) then
			c = c + 1
			if IsActiveInChannel(ply, channel.uniqueID) then cm = cm + 1 end
		end
	end
...
function YRPCountPassiveChannels(ply)
	...
	for i, channel in SortedPairsByMemberValue(GetGlobalYRPTable("yrp_voice_channels", {}), "int_position", false) do
		if IsInChannel(ply, channel.uniqueID, true) then
			c = c + 1
			if IsInChannel(ply, channel.uniqueID) then cm = cm + 1 end
		end
	end
...
net.Receive("nws_yrp_mutemic_channel", function(len, ply)
	...
	uid = uid or "0"
	uid = tonumber(uid)
	if not ply:GetYRPBool("yrp_voice_channel_mute_" .. uid, false) then ply:SetYRPBool("yrp_voice_channel_mute_" .. uid, true) end
...
net.Receive("nws_yrp_mute_channel", function(len, ply)
	local uid = YRP_SQL_ID(net.ReadString())
	if not ply:GetYRPBool("yrp_voice_channel_mutemic_" .. uid, false) then ply:SetYRPBool("yrp_voice_channel_mutemic_" .. uid, true) end
...
net.Receive("nws_yrp_ToggleVoiceMenu", function(len, ply) ply:SetYRPBool("yrp_ToggleVoiceMenu", not ply:GetYRPBool("yrp_ToggleVoiceMenu", true)) end)
...
hook.Add("YRP_SQLDBREADY_COMMUNICATION", "yrp_VOICE_PlayerCanHearPlayersVoice", function()
	timer.Simple(0.1, function()
...
function YRPCheckAddons(force)
	...
	if force then YRPHR(Color(100, 100, 255)) end
end

hook.Add("PostGamemodeLoaded", "yrp_PostGamemodeLoaded_CheckAddons", function() timer.Simple(2.1, function() YRPCheckAddons(true) end) end)
...
function YRPRemoveEmptyLines(str)
	...
	for i, v in pairs(tab) do
		if not strEmpty(v) then table.insert(newtab, v) end
	end
...
function YRPRemoveCommentsShort(str)
	local tab = string.Explode("\n", str, false)
	for i, v in pairs(tab) do
		if string.StartWith(v, "--") then tab[i] = "" end
	end
...
function YRPImportDarkrp(str, name)
	...
	if str and name then
		...
		if err and string.find(err, "table index is nil", 1, true) == nil then MsgC(Color(0, 255, 0), "ERROR: ", err, "\n") end
	else
...
net.Receive("yrp_exploiter_detected", function(len, ply)
	local msg = string.format("Detected Exploiter: %s [%s]", ply:SteamName(), ply:SteamID())
	for i = 0, 4 do
		YRP:msg("note", msg)
	end

	if IsValid(ply) then ply:Kick("EXPLOITER DETECTED") end
end)
