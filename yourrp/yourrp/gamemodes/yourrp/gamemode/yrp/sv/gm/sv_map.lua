...
function YRPTryNewPos(dir, size, space, tmpX, tmpY, tmpZ)
	...
	for i = dir, size, space do
		if util.IsInWorld(Vector(tmpX or i, tmpY or i, tmpZ or i)) and _tmpEnd < _fails then
			_result = i
			if _tmpEnd > 0 then _tmpEnd = 0 end
		else
			_tmpEnd = _tmpEnd + 1
...
function YRPGetCoords()
	...
	for k, v in pairs(ents.GetAll()) do
		if v and YRPEntityAlive(v) and (v:GetClass() == "info_player_teamspawn" or v:GetClass() == "info_player_terrorist" or v:GetClass() == "info_player_counterterrorist") then _hasNoSpawnpoints = true end
	end

	for k, v in pairs(ents.GetAll()) do
		if IsValid(v) then
			if _hasNoSpawnpoints then
				if v:GetClass() == "info_player_start" then YRPSearchCoords(v) end
			else
			...
			if v:GetClass() == "prop_door_rotating" then YRPSearchCoords(v) end
			if v:GetClass() == "func_door" then YRPSearchCoords(v) end
			if v:GetClass() == "func_door_rotating" then YRPSearchCoords(v) end
		end
	end
