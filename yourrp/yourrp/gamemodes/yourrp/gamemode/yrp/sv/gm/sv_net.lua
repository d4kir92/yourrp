...
net.Receive("nws_yrp_updateServer", function(len, ply)
	...
	local _result = YRP_SQL_UPDATE("yrp_general", {
	...
	timer.Create("timerRestartServer", 1, 0, function()
		...
		if countdown == 0 then message = "Server is updating." end
...
concommand.Add("darkrp", function(ply, cmd, args)
	...
	elseif args[1] and args[1] == "name" then
		...
		if YRPEntityAlive(ply) then ply:SetRPName(newrpname, "darkrp name") end
	...
	elseif args[1] and args[1] == "drop" then
		...
		if _weapon ~= nil and PlayersCanDropWeapons() then
			if YRPEntityAlive(_weapon) then ply:DropSWEP(_weapon:GetClass()) end
		else
	...
	elseif args[1] and jobByCmd[string.upper(args[1])] then
		...
		if jobtab and YRPEntityAlive(ply) then YRP:TryGetRole(ply, jobtab.uniqueID, 1, {}) end
	elseif args[1] and unhandled[args[1]] == nil then
		if args[1] and not string.StartsWith(string.lower(args[1]), "team_") then YRP:msg("error", "[darkrp] console command: args[1]: " .. tostring(args[1]) .. " args[2]: " .. tostring(args[2]) .. " args[3]: " .. tostring(args[3])) end
	end
end)
...
concommand.Add("yrp_usergroup", function(ply, cmd, args) YRPChangeUserGroup(ply, cmd, args) end)
concommand.Add("yrp_givelicense", function(ply, cmd, args)
	if not YRPConCommandAccess(ply, "yrp_givelicense") then return end
	if #args ~= 2 then YRP:msg("note", "to much/less commands") end
	local name = args[1]
	local lname = args[2]
