...
local yrp_general = {}
hook.Add("YRP_SQLDBREADY_GENERAL_DB", "yrp_general", function()
	--[[ Server Settings ]]
	--
	...
	if YRP_SQL_SELECT(DATABASE_NAME, "*", "uniqueID = '1'") == nil then YRP_SQL_INSERT_INTO_DEFAULTVALUES(DATABASE_NAME) end
end)

hook.Add("YRP_SQLDBREADY_GENERAL", "yrp_general", function()
...
function AddToHandler_General(ply)
	if not table.HasValue(HANDLER_GENERAL, ply) then table.insert(HANDLER_GENERAL, ply) end
end
...
net.Receive("nws_yrp_disconnect_Settings_General", function(len, ply) RemFromHandler_General(ply) end)
...
function YRPGetMoneyModel()
	...
	if model and strEmpty(model) then model = "models/props/cs_assault/money.mdl" end
...
net.Receive("nws_yrp_update_int_server_debug_tick", function(len, ply)
	...
	if isnumber(tonumber(int)) then GeneralUpdateInt(ply, "nws_yrp_update_int_server_debug_tick", "int_server_debug_tick", int) end
end)
...
net.Receive("nws_yrp_update_int_ttlsweps", function(len, ply)
	local int = net.ReadString()
	if isnumber(tonumber(int)) then GeneralUpdateInt(ply, "nws_yrp_update_int_ttlsweps", "int_ttlsweps", int) end
end)

YRP:AddNetworkString("nws_yrp_update_int_afkkicktime")
net.Receive("nws_yrp_update_int_afkkicktime", function(len, ply)
	local int = net.ReadString()
	if isnumber(tonumber(int)) then GeneralUpdateInt(ply, "nws_yrp_update_int_afkkicktime", "int_afkkicktime", int) end
end)
...
net.Receive("nws_yrp_update_bool_building_system_3d", function(len, ply)
	local b = btn(net.ReadBool())
...
net.Receive("nws_yrp_update_bool_securitylevel_system", function(len, ply)
	local b = btn(net.ReadBool())
...
net.Receive("nws_yrp_update_bool_allbuildingsunlocked", function(len, ply)
	local b = btn(net.ReadBool())
...
net.Receive("nws_yrp_update_int_voice_max_range", function(len, ply)
	local int = net.ReadString()
	if isnumber(tonumber(int)) then GeneralUpdateInt(ply, "nws_yrp_update_int_voice_max_range", "int_voice_max_range", int) end
end)
...
net.Receive("nws_yrp_update_bool_yrp_chat_commands", function(len, ply)
	local b = btn(net.ReadBool())
...
net.Receive("nws_yrp_update_int_yrp_chat_range_local", function(len, ply)
	local int = net.ReadString()
	if isnumber(tonumber(int)) then GeneralUpdateInt(ply, "nws_yrp_update_int_yrp_chat_range_local", "int_yrp_chat_range_local", int) end
end)
...
net.Receive("nws_yrp_update_bool_yrp_money", function(len, ply)
...
net.Receive("nws_yrp_update_int_logouttime", function(len, ply)
	local int = net.ReadString()
	if isnumber(tonumber(int)) then GeneralUpdateInt(ply, "nws_yrp_update_int_logouttime", "int_logouttime", int) end
end)
...
net.Receive("nws_yrp_update_int_deathtimestamp_min", function(len, ply)
	local int = net.ReadString()
	if isnumber(tonumber(int)) then GeneralUpdateInt(ply, "nws_yrp_update_int_deathtimestamp_min", "int_deathtimestamp_min", int) end
end)
...
net.Receive("nws_yrp_update_int_deathtimestamp_max", function(len, ply)
	local int = net.ReadString()
	if isnumber(tonumber(int)) then GeneralUpdateInt(ply, "nws_yrp_update_int_deathtimestamp_max", "int_deathtimestamp_max", int) end
end)
...
net.Receive("nws_yrp_getsitestaff", function(len, ply)
	...
	for i, pl in pairs(player.GetAll()) do
		if pl:HasAccess("nws_yrp_getsitestaff", true) then table.insert(staff, pl) end
	end
...
net.Receive("nws_yrp_tp_tpto_steamid", function(len, ply)
	...
	if _target and IsValid(_target) then YRPTeleportToPoint(ply, _target:GetPos()) end
end)
...
net.Receive("nws_yrp_tp_bring_steamid", function(len, ply)
	...
	local steamid = net.ReadString()
	local _target = YRPGetPlayerBySteamID(steamid)
	if _target and IsValid(_target) then YRPTeleportToPoint(_target, ply:GetPos()) end
end)
