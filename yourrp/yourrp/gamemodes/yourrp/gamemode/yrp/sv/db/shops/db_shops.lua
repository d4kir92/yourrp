...
hook.Add("YRP_SQLDBREADY_GAMEPLAY_DB", "yrp_shops", function() YRP_SQL_ADD_COLUMN(DATABASE_NAME, "name", "TEXT DEFAULT 'UNNAMED'") end)
...
net.Receive("nws_yrp_get_shops", function(len, ply) if ply:CanAccess("bool_shops") then send_shops(ply) end end)
...
function HasShopPermanent(tab)
	...
	local _nw = {}
	if _cats ~= nil then _nw = _cats end
...
net.Receive("nws_yrp_shop_get_all_tabs", function(len, ply)
	...
	local _nw = {}
	if _tabs ~= nil then _nw = _tabs end
