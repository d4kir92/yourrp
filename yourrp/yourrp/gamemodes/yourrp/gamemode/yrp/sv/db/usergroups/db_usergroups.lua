...
hook.Add("YRP_SQLDBREADY_GENERAL_DB", "yrp_usergroups", function()
...
hook.Add("YRP_SQLDBREADY_GENERAL_UPDATE", "yrp_usergroups", function()
			...
			for _i, _ug in pairs(yrp_usergroups) do
				...
				if IsNotNilAndNotFalse(tmp) and #tmp > 1 then
					for i, ug in pairs(tmp) do
						if i > 1 then YRP_SQL_DELETE_FROM(DATABASE_NAME, "uniqueID = '" .. ug.uniqueID .. "'") end
					end
				end
			end
		end
...
function AddToHandler_UserGroups(ply)
	if not table.HasValue(HANDLER_USERGROUPS, ply) then table.insert(HANDLER_USERGROUPS, ply) end
end
...
function RemFromHandler_UserGroup(ply, uid)
	...
	if table.HasValue(HANDLER_USERGROUP[uid], ply) then table.RemoveByValue(HANDLER_USERGROUP[uid], ply) end
end

function AddToHandler_UserGroup(ply, uid)
	HANDLER_USERGROUP[uid] = HANDLER_USERGROUP[uid] or {}
	if not table.HasValue(HANDLER_USERGROUP[uid], ply) then table.insert(HANDLER_USERGROUP[uid], ply) end
end
...
net.Receive("nws_yrp_connect_Settings_UserGroup", function(len, ply)
	...
	if IsNotNilAndNotFalse(_tmp) then _tmp = _tmp[1] end
...
function UGUpdateInt(ply, uid, name, value)
	...
	if IsNotNilAndNotFalse(ug) then
		...
		for i, pl in pairs(player.GetAll()) do
			if string.lower(pl:GetUserGroup()) == ug.string_name then pl:SetYRPInt(name, value) end
		end
	end
end
...
function UGCheckBox(ply, uid, name, value)
	...
	if IsNotNilAndNotFalse(ug) then
		...
		ug.string_name = string.lower(ug.string_name)
		for i, pl in pairs(player.GetAll()) do
			if string.lower(pl:GetUserGroup()) == ug.string_name then pl:SetYRPBool(name, tobool(value)) end
		end
	end
end
...
function YRPNotiToPlyDisallowed(ply, msg)
	if not table.HasValue(antinoti1spam, ply) then
		...
		net.WriteString(msg)
		net.Send(ply)
		timer.Simple(5, function() table.RemoveByValue(antinoti1spam, ply) end)
	end
end
...
function YRPNotiToPly(msg, ply)
	if msg and not table.HasValue(antinoti2spam, ply) then
		...
		timer.Simple(5, function() table.RemoveByValue(antinoti2spam, ply) end)
	end
end
...
function HasUseFunction(ent)
	if IsEntity(ent) and ent.Use ~= nil then ent:SetYRPBool("yrp_has_use", true) end
end
		...
		if IsNotNilAndNotFalse(_tmp, "PlayerSpawnProp failed") then
			...
			if tobool(_tmp.bool_props) then
				if PropBlacklisted(mdl) then
					if not pl:HasAccess("spawnpropblacklistnoti", true) then YRP:msg("note", pl:Nick() .. " [" .. string.lower(pl:GetUserGroup()) .. "] has no right to spawn blacklisted prop.") end
...
local toolantispam = {}
hook.Add("CanTool", "      yrp_can_tool", function(pl, tr, toolname, tool, button)
			...
			if tr and tr.Entity and IsValid(tr.Entity) and tr.Entity:GetRPOwner() then
				...
				if Owner == pl or not YRPEntityAlive(Owner) or pl:HasAccess("yrp_can_tool", true) then
					...
					if not table.HasValue(toolantispam, pl) then
						...
						timer.Simple(2, function() if YRPEntityAlive(pl) then table.RemoveByValue(toolantispam, pl) end end)
					end
					return false
				end
				return true
...
function Player:UserGroupLoadout()
	...
	if IsNotNilAndNotFalse(UG) then
		...
		for name, amount in pairs(ammos) do
			...
			if ammo then self:SetAmmo(ammo + amount, name) end
		end
		...
		if _licenseIDs then
			...
			for i, lic in pairs(_licenseIDs) do
				if tonumber(lic) ~= nil then GiveLicense(self, lic) end
			end
...
net.Receive("nws_yrp_get_perma_props", function(len, ply)
	...
	local tab = {}
	if YRP_SQL_TABLE_EXISTS("permaprops", "nws_yrp_get_perma_props") then tab = YRP_SQL_SELECT("permaprops", "*", nil) end
...
net.Receive("nws_yrp_permaprops_remove", function(len, ply)
	...
	if YRP_SQL_TABLE_EXISTS("permaprops", "nws_yrp_permaprops_remove") then YRP_SQL_DELETE_FROM("permaprops", "id = '" .. ppid .. "'") end
end)
...
net.Receive("nws_yrp_get_perma_props2", function(len, ply)
	if not ply:GetYRPBool("bool_permaprops", false) then return end
	local tab = {}
	if YRP_SQL_TABLE_EXISTS("permaprops_system", "nws_yrp_get_perma_props2") then tab = YRP_SQL_SELECT("permaprops_system", "*", nil) end
	ply.ppid = ply.ppid or 0
	if IsNotNilAndNotFalse(tab) then
		ply.ppid = ply.ppid + 1
		local ppid = ply.ppid
...
net.Receive("nws_yrp_permaprops_remove2", function(len, ply)
	if not ply:GetYRPBool("bool_permaprops", false) then return end
	local ppid = YRP_SQL_ID(net.ReadString())
	if YRP_SQL_TABLE_EXISTS("permaprops_system", "nws_yrp_permaprops_remove2") then YRP_SQL_DELETE_FROM("permaprops_system", "id = '" .. ppid .. "'") end
end)
...
timer.Create("YRPCheckUGChanged", 1, 0, function()
	local _, err = pcall(YRPCheckUGChanged)
	if err then YRPMsg(err) end
end)
