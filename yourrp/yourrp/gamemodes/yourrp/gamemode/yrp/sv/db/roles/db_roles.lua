...
function MoveUnusedGroups()
	...
	if count > 0 then MoveUnusedGroups() end
end
...
function MoveUnusedRolesToDefault()
	...
	if changed then YRP:msg("note", "Moved unused roles to the default group") end
end

hook.Add("YRP_SQLDBREADY_GENERAL_DB", "yrp_ply_roles", function()
...
hook.Add("YRP_SQLDBREADY_GENERAL_UPDATE", "yrp_ply_roles", function()
	YRP_SQL_UPDATE(DATABASE_NAME, {
		["int_uses"] = 0
	}, nil)
...
hook.Add("YRP_SQLDBREADY_GENERAL", "yrp_ply_roles", function()
	...
	if IsNotNilAndNotFalse(_init_ply_roles) then yrp_roles = _init_ply_roles[1] end
...
function YRPConvertToDarkRPJob(tab)
	...
	if string.find(pms, ",", 1, true) then pms = string.Explode(",", pms) end
	...
	local _weapons = {}
	if tab.string_sweps then _weapons = string.Explode(",", tab.string_sweps) end
	...
	if IsNotNilAndNotFalse(gname) then gname = gname[1].string_name end
	_job.category = gname or "invalid group"
	_job.command = YRPConvertToDarkRPJobName(tab.string_name)
	if not strEmpty(tab.string_identifier) then _job.command = tab.string_identifier end
...
function YRPBuildDarkrpTeams()
	...
	if IsNotNilAndNotFalse(drp_allroles) then
		for i, role in pairs(drp_allroles) do
			...
			if not strEmpty(role.string_identifier) then teamname = role.string_identifier end
...
function Player:DRPSendTeamsToPlayer()
	...
	for i, role in pairs(RPExtraTeams) do
		if IsValid(self) then
			if type(role.model) == "string" then role.model = string.Explode(",", role.model) end
			if type(role.weapons) == "string" then role.weapons = string.Explode(",", role.weapons) end
	...
	if IsNotNilAndNotFalse(TEAMS) then
		for i, cat in pairs(CATEGORIES.jobs) do
			...
			for id, role in pairs(TEAMS) do
				if role.int_groupID == cat.uniqueID then table.insert(cat.members, role) end
			end
		end
	else
...
-- darkrp
timer.Simple(10, function() if GAMEMODE.DarkRPFinishedLoading then GAMEMODE:DarkRPFinishedLoading() end end)
...
function YRPSubscribeRoleList(ply, gro, pre)
	if HANDLER_GROUPSANDROLES["roleslist"][gro] == nil then HANDLER_GROUPSANDROLES["roleslist"][gro] = {} end
	if HANDLER_GROUPSANDROLES["roleslist"][gro][pre] == nil then HANDLER_GROUPSANDROLES["roleslist"][gro][pre] = {} end
	if not table.HasValue(HANDLER_GROUPSANDROLES["roleslist"][gro][pre], ply) then table.insert(HANDLER_GROUPSANDROLES["roleslist"][gro][pre], ply) end
end

function YRPUnsubscribeRoleList(ply, gro, pre)
	if HANDLER_GROUPSANDROLES["roleslist"][gro] == nil then HANDLER_GROUPSANDROLES["roleslist"][gro] = {} end
	if HANDLER_GROUPSANDROLES["roleslist"][gro][pre] == nil then HANDLER_GROUPSANDROLES["roleslist"][gro][pre] = {} end
	if table.HasValue(HANDLER_GROUPSANDROLES["roleslist"][gro][pre], ply) then table.RemoveByValue(HANDLER_GROUPSANDROLES["roleslist"][gro][pre], ply) end
end

function YRPSubscribeRole(ply, uid)
	if HANDLER_GROUPSANDROLES["roles"][uid] == nil then HANDLER_GROUPSANDROLES["roles"][uid] = {} end
	if not table.HasValue(HANDLER_GROUPSANDROLES["roles"][uid], ply) then table.insert(HANDLER_GROUPSANDROLES["roles"][uid], ply) end
end

function YRPUnsubscribeRole(ply, uid)
	if HANDLER_GROUPSANDROLES["roles"][uid] == nil then HANDLER_GROUPSANDROLES["roles"][uid] = {} end
	if table.HasValue(HANDLER_GROUPSANDROLES["roles"][uid], ply) then table.RemoveByValue(HANDLER_GROUPSANDROLES["roles"][uid], ply) end
end
...
function SendRoleList(ply, gro, pre)
	...
	if gro ~= nil and pre ~= nil then
		...
		if not IsNotNilAndNotFalse(tbl_roles) then tbl_roles = {} end
		local headername = "NOT FOUND"
		if pre > 0 then
			headername = YRP_SQL_SELECT(DATABASE_NAME, "*", "uniqueID = '" .. pre .. "'")
			if YRPWORKED(headername) then headername = headername[1].string_name end
		else
		...
		if IsNotNilAndNotFalse(role) then
			...
			for name, value in pairs(role) do
				if name ~= "uniqueID" and name ~= "int_removeable" then
					if name == "int_groupID" then value = guid end
			...
			if guid and role.int_prerole then SendRoleList(nil, guid, role.int_prerole) end
		else
...
net.Receive("nws_yrp_settings_subscribe_role", function(len, ply)
	...
	if not IsNotNilAndNotFalse(roles) then roles = {} end
...
net.Receive("nws_yrp_settings_delete_role", function(len, ply)
	...
	if IsNotNilAndNotFalse(role) then
		...
		if not IsNotNilAndNotFalse(roledeleted) then YRP:msg("note", "Role deleted (uid: " .. tostring(uid) .. " )") end
		local siblings = YRP_SQL_SELECT(DATABASE_NAME, "*", "int_groupID = '" .. role.int_groupID .. "'")
		if IsNotNilAndNotFalse(siblings) then
			for i, sibling in pairs(siblings) do
				sibling.int_position = tonumber(sibling.int_position)
...
net.Receive("nws_yrp_get_all_role_customflags", function(len, ply)
	...
	if not IsNotNilAndNotFalse(allflags) then allflags = {} end
...
net.Receive("nws_yrp_add_role_flag", function(len, ply)
	...
	if not table.HasValue(customflags, tostring(fuid)) then
		...
		for i, v in pairs(customflags) do
			if v ~= "" then table.insert(oldflags, v) end
		end
	...
	if IsNotNilAndNotFalse(role) then
		...
		for i, id in pairs(string_playermodels) do
			...
			if IsNotNilAndNotFalse(pms) then
				...
				if name == "" or name == " " then name = pms.string_models end
...
net.Receive("nws_yrp_get_all_playermodels", function(len, ply)
	...
	if not IsNotNilAndNotFalse(allpms) then allpms = {} end
			...
			for y, role in pairs(roles) do
				...
				if rolepms then
					for z, rolepm in pairs(rolepms) do
						if tonumber(pm.uniqueID) == tonumber(rolepm) then pm.uses = pm.uses + 1 end
					end
				end
			end
		end
...
function AddPlayermodelToRole(ruid, muid)
	...
	if not table.HasValue(pms, tostring(muid)) then
		...
		for i, pm in pairs(pms) do
			if not strEmpty(pm) then table.insert(oldpms, pm) end
		end
...
function RemPlayermodelFromRole(ruid, muid)
	...
	local oldpms = {}
	for i, v in pairs(pms) do
		if not strEmpty(v) then table.insert(oldpms, v) end
	end
...
net.Receive("nws_yrp_rem_role_playermodel", function(len, ply)
	...
	if IsNotNilAndNotFalse(test) then pms = test[1].string_models end
...
function SendSweps(uid)
	...
	if IsNotNilAndNotFalse(role) then
		...
		for i, swep in pairs(sweps) do
			if not strEmpty(swep) then table.insert(nettab, swep) end
		end
	...
	if IsNotNilAndNotFalse(role) then
		...
		if not table.HasValue(sweps, tostring(swepcn)) then
			...
			for i, v in pairs(sweps) do
				if not strEmpty(v) then table.insert(oldsweps, v) end
			end
...
function SendNDSweps(uid)
	...
	if IsNotNilAndNotFalse(role) then
		...
		for i, ndswep in pairs(ndsweps) do
			if not strEmpty(ndswep) then table.insert(nettab, ndswep) end
		end
...
function AddNDSwepToRole(ruid, ndswepcn)
	...
	if not table.HasValue(ndsweps, tostring(ndswepcn)) then
		...
		for i, v in pairs(ndsweps) do
			if not strEmpty(v) then table.insert(oldndsweps, v) end
		end
...
function RemLicenseFromRole(ruid, muid)
	...
	for i, v in pairs(lis) do
		if not strEmpty(v) then table.insert(oldlis, v) end
	end
	...
	if IsNotNilAndNotFalse(role) then
		...
		for i, li in pairs(lis) do
			if li ~= "" then
				...
				if not IsNotNilAndNotFalse(found) then RemLicenseFromRole(ruid, li) end
			end
		end
	end
end
...
net.Receive("nws_yrp_get_all_licenses", function(len, ply)
	...
	if not IsNotNilAndNotFalse(alllis) then alllis = {} end
	...
	if IsNotNilAndNotFalse(role) then
		...
		for i, li in pairs(lis) do
			if li ~= "" then
				...
				if not IsNotNilAndNotFalse(found) then RemSpecializationFromRole(ruid, li) end
			end
		end
	end
end
	...
	if YRPEntityAlive(target) and target:IsPlayer() then
		...
		if IsNotNilAndNotFalse(chatab) then
			...
			if IsNotNilAndNotFalse(tmpT) and IsNotNilAndNotFalse(tmpTable) then
					...
					if IsNotNilAndNotFalse(tmpTableSearch) then
						...
						while tmpSearch do
							if IsNotNilAndNotFalse(tmpTableSearch) then
								...
								if tmpPromote and tmpDemote then tmpSearch = false end
							end
				...
				if not strEmpty(tmpTable.string_specializations) then tmpHasSpecs = true end
...
function removeFromWhitelist(SteamID, roleID)
	...
	if _result ~= nil then YRP_SQL_DELETE_FROM("yrp_role_whitelist", "uniqueID = " .. _result[1].uniqueID) end
end
...
net.Receive("nws_yrp_promotePlayer", function(len, ply)
	...
	if tonumber(tmpTableInstructorRole.bool_instructor) == 1 then
		...
		if tmpTableTargetPromoteRole.uniqueID ~= tmpTableInstructorRole.uniqueID then
			...
			if IsValid(target) then YRPSetRole("nws_yrp_promotePlayer", target, tmpTableTargetPromoteRole.uniqueID, true) end
...
net.Receive("nws_yrp_demotePlayer", function(len, ply)
	...
	if tonumber(tmpTableInstructorRole.bool_instructor) == 1 then
		...
		if IsValid(target) then YRPSetRole("nws_yrp_demotePlayer", target, tmpTableTargetDemoteRole.uniqueID) end
...
function YRPGetFirstRankUID(ruid)
	...
	while Search do
		...
		tries = tries - 1
		if tries <= 0 then Search = false end
	end
	return resultuid
end
...
function GetRoleTable(rid)
	...
	if IsNotNilAndNotFalse(result) then result = result[1] end
	return result
end
...
net.Receive("nws_yrp_hasnext_ranks", function(len, ply)
	...
	local rols = YRP_SQL_SELECT(DATABASE_NAME, "*", "int_prerole = '" .. ruid .. "'")
	local has = false
	if IsNotNilAndNotFalse(rols) then has = true end
