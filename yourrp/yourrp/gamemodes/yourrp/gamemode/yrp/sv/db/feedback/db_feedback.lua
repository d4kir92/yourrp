...
hook.Add("YRP_SQLDBREADY_GENERAL_DB", "yrp_feedback", function()
	...
	if ply:CanAccess("bool_feedback") then
		...
		if IsNotNilAndNotFalse(_result) then
			for i, t in pairs(_result) do
				...
				if t.status == "closed" and os.time() - deleteafter > t.string_timestamp then YRP_SQL_DELETE_FROM(DATABASE_NAME, "uniqueID = '" .. t.uniqueID .. "'") end
			end
		end

		_result = YRP_SQL_SELECT(DATABASE_NAME, "*", nil)
		if _result == nil or _result == false then _result = {} end
...
net.Receive("nws_yrp_fb_movetoopen", function(len, ply)
