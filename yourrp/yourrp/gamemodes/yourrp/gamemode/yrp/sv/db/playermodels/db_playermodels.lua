...
function YRP_CheckAndRemoveUnusedPlayermodels()
		...
		for i, role in pairs(roles) do
			if role.string_playermodels then
				...
				for j, pm in pairs(pms) do
					if not strEmpty(pm) and not table.HasValue(usedpms, tonumber(pm)) then table.insert(usedpms, tonumber(pm)) end
				end
			end
		end
	end
	...
	if IsNotNilAndNotFalse(dbpms) then
		for i, pm in pairs(dbpms) do
			...
			if not table.HasValue(usedpms, pm.uniqueID) then YRP_SQL_DELETE_FROM(DATABASE_NAME, "uniqueID = '" .. pm.uniqueID .. "'") end
		end
	end
end

hook.Add("YRP_SQLDBREADY_VISUAL_DB", "yrp_playermodels", function()
...
hook.Add("YRP_SQLDBREADY_VISUAL_UPDATE", "yrp_playermodels", function() timer.Simple(1, function() YRP_CheckAndRemoveUnusedPlayermodels() end) end)
