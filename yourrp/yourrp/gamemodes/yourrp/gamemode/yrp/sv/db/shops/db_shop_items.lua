...
hook.Add("YRP_SQLDBREADY_GAMEPLAY_DB", "yrp_shop_items", function()
...
function send_shop_items(ply, uid)
	...
	local _nw = _s_items
	if _nw == nil then _nw = {} end
	net.Start("nws_yrp_get_shop_items")
	net.WriteTable(_nw)
	net.Send(ply)
end

net.Receive("nws_yrp_get_shop_items", function(len, ply)
...
net.Receive("nws_yrp_shop_item_rem", function(len, ply)
...
net.Receive("nws_yrp_shop_item_edit_name", function(len, ply)
...
net.Receive("nws_yrp_shop_item_edit_desc", function(len, ply)
...
net.Receive("nws_yrp_shop_item_edit_price", function(len, ply)
...
net.Receive("nws_yrp_shop_item_edit_level", function(len, ply)
...
net.Receive("nws_yrp_shop_item_edit_quan", function(len, ply)
...
net.Receive("nws_yrp_shop_item_edit_cool", function(len, ply)
...
net.Receive("nws_yrp_shop_item_edit_lice", function(len, ply)
...
net.Receive("nws_yrp_shop_item_edit_perm", function(len, ply)
...
net.Receive("nws_yrp_shop_get_items_storage", function(len, ply)
	local _uid = YRP_SQL_ID(net.ReadString())
	...
	if _cha_perm ~= nil and _cha_perm ~= false then
		...
		for i, item in pairs(_cha_perm) do
			...
			if _item ~= nil and _item ~= false then table.insert(_nw, _item[1]) end
		end
...
net.Receive("nws_yrp_shop_get_items", function(len, ply)
	local _uid = YRP_SQL_ID(net.ReadString())
	local _items = YRP_SQL_SELECT(DATABASE_NAME, "*", "categoryID = '" .. _uid .. "'")
	local _nw = {}
	if _items ~= nil then _nw = _items end
...
net.Receive("nws_yrp_shop_item_edit_base", function(len, ply)
...
function SpawnVehicle(item, pos, ang)
	...
	if vehicle.ClassName ~= nil then
		...
		if vehicle.Skin ~= "-1" then car:SetSkin(vehicle.Skin) end
...
function YRPSpawnItem(ply, item, duid, count, itemColor)
	...
	if not tr.Hit then foundpos = true end
				...
				for i = 0, 360 - 15, 15 do
					if not foundpos then
						for hei = 0, 6 do
							...
							if hei > 0 then pos = pos + Vector(0, 0, hei / 5 * math.abs(maxs.z - mins.z)) end
							tr = util.TraceHull({
								start = pos,
								endpos = pos,
								mins = mins,
	...
	if tr2.Hit then
		...
		if ENT ~= nil then
			...
			if IsValid(ent) then
				...
				if not hasstorage then ent:SetPos(ply:GetPos() + ply:GetForward() * 64 + ply:GetUp() * 64) end
			...
			local vehicle = nil
			if list.Get("simfphys_vehicles") and item.ClassName then vehicle = list.Get("simfphys_vehicles")[item.ClassName] end
			if vehicle and simfphys and simfphys.SpawnVehicle then
				...
				if IsValid(ent) then
					...
					for i, v in pairs(list.Get("Vehicles")) do
						if v.Model == item.WorldModel then veh = v end
					end
		...
		if YRPEntityAlive(ent) and TARGETANG then ent:SetAngles(TARGETANG) end
	else
...
net.Receive("nws_yrp_item_buy", function(len, ply)
	...
	if IsNotNilAndNotFalse(_item) then
		...
		if count > 1 and _item.type == "weapons" then totalPrice = _item.price * count end
		...
		if ply:canAfford(totalPrice) then
			...
			timer.Simple(0.3, function() YRPPlyUpdateStorage(ply) end)
			...
			elseif _item.type == "roles" then
				...
				if _spawned then
					if YRPEntityAlive(ent) then
						...
						if itemColor then ent:SetColor(StringToColor(itemColor)) end
						if ent:IsVehicle() then AddVehicle(ent, ply, _item) end
					end

					if _item.cooldown > 0 then ply:SetYRPFloat("buy_ts" .. _item.uniqueID, CurTime() + _item.cooldown) end
				else
			...
			if tonumber(_item.permanent) == 1 then
				...
				for i, item in pairs(_stor) do
					if item == "" then table.RemoveByValue(_stor, "") end
				end

				if not table.HasValue(_stor, _item.uniqueID) then table.insert(_stor, _item.uniqueID) end
				_stor = string.Implode(",", _stor)
				local _result = YRP_SQL_UPDATE("yrp_characters", {
					["storage"] = _stor
				}, "uniqueID = '" .. ply:CharID() .. "'")
			end
...
net.Receive("nws_yrp_item_spawn", function(len, ply)
...
net.Receive("nws_yrp_item_despawn", function(len, ply)
	local _tab = net.ReadTable()
	...
	if _item ~= nil then
		...
		if _alive and YRPEntityAlive(ent) then ent:Remove() end
	end
end)
