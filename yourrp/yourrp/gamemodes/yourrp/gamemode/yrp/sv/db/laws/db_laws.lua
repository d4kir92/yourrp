...
hook.Add("YRP_SQLDBREADY_GAMEPLAY_DB", "yrp_laws", function()
	YRP_SQL_ADD_COLUMN(DATABASE_NAME, "string_lawsymbol", "TEXT DEFAULT '§'")
	YRP_SQL_ADD_COLUMN(DATABASE_NAME, "string_laws", "TEXT DEFAULT ''")
	if YRP_SQL_SELECT(DATABASE_NAME, "*", "uniqueID = '1'") == nil then YRP_SQL_INSERT_INTO(DATABASE_NAME, "string_lawsymbol, string_laws", "'§', ''") end
end)

hook.Add("YRP_SQLDBREADY_GAMEPLAY", "yrp_laws", function()
...
net.Receive("nws_yrp_get_laws", function(len, ply)
	...
	if IsNotNilAndNotFalse(laws) then
		...
		if not IsNotNilAndNotFalse(buildings) then buildings = {} end
