...
function dealer_add(ply)
	...
	if _db_sel != nil then
		_db_sel = _db_sel[1]
		local _db_upd = YRP_SQL_UPDATE(DATABASE_NAME, {["name"] = "Unnamed Dealer"}, "uniqueID = " .. _db_sel.uniqueID)
...
net.Receive( "dealer_add_tab", function(len, ply)
	...
	if _dealer != nil then
		...
		local _up = YRP_SQL_UPDATE(DATABASE_NAME, {["tabs"] = _tabs}, "uniqueID = " .. _dealer_uid)
	end
end)
...
net.Receive( "dealer_rem_tab", function(len, ply)
	...
	if _dealer != nil then
		...
		YRP_SQL_UPDATE(DATABASE_NAME, {["tabs"] = _dealer.tabs}, "uniqueID = " .. _dealer_uid)
	end
end)
...
net.Receive( "dealer_edit_storagepoints", function(len, ply)
	...
	local _dealer = YRP_SQL_UPDATE(DATABASE_NAME, {["storagepoints"] = _dealer_storagepoints}, "uniqueID = " .. _dealer_uid)
end)
