...
hook.Add("YRP_SQLDBREADY_GENERAL_DB", "yrp_role_whitelist", function()
...
function sendRoleWhitelist(ply)
	...
	if not IsNotNilAndNotFalse(tabW) then tabW = {} end
	for i, v in pairs(tabW) do
		...
		if v.groupID > 0 then
			...
			if not IsNotNilAndNotFalse(tabG) then YRP_SQL_DELETE_FROM(DATABASE_NAME, "uniqueID = '" .. v.uniqueID .. "'") end
		end

		if v.roleID > 0 then
			local tabR = YRP_SQL_SELECT("yrp_ply_roles", "*", "uniqueID = '" .. v.roleID .. "'")
			if not IsNotNilAndNotFalse(tabR) then YRP_SQL_DELETE_FROM(DATABASE_NAME, "uniqueID = '" .. v.uniqueID .. "'") end
		end
	end

	if ply:CanAccess("bool_whitelist") then
		local _tmpWhiteList = YRP_SQL_SELECT("yrp_role_whitelist", "*", nil)
		if not IsNotNilAndNotFalse(_tmpWhiteList) then _tmpWhiteList = {} end
...
net.Receive("nws_yrp_whitelistPlayer", function(len, ply)
	...
	if IsNotNilAndNotFalse(DBRole) then
		...
		YRP_SQL_INSERT_INTO("yrp_role_whitelist", "SteamID, nick, groupID, roleID, date, status, name", "'" .. _SteamID .. "', " .. YRP_SQL_STR_IN(_nick) .. ", " .. _groupID .. ", " .. roleID .. ", '" .. dat .. "', " .. YRP_SQL_STR_IN(status) .. ", " .. YRP_SQL_STR_IN(name) .. "")
...
net.Receive("nws_yrp_whitelistPlayerGroup", function(len, ply)
	...
	if IsNotNilAndNotFalse(DBGroup) then DBGroup = DBGroup[1] end
	local dat = util.DateStamp()
	local status = "Manually by " .. ply:SteamName()
	local name = target:SteamName()
	YRP_SQL_INSERT_INTO("yrp_role_whitelist", "SteamID, nick, groupID, date, status, name", "'" .. _SteamID .. "', " .. YRP_SQL_STR_IN(_nick) .. ", " .. groupID .. ", '" .. dat .. "', " .. YRP_SQL_STR_IN(status) .. ", " .. YRP_SQL_STR_IN(name) .. "")
...
net.Receive("nws_yrp_whitelistPlayerAll", function(len, ply)
	...
	local status = "Manually by " .. ply:SteamName()
	local name = target:SteamName()
	YRP_SQL_INSERT_INTO("yrp_role_whitelist", "SteamID, nick, roleID, groupID, date, status, name", "'" .. _SteamID .. "', " .. YRP_SQL_STR_IN(_nick) .. ", " .. "-1" .. ", " .. "-1" .. ", '" .. dat .. "', " .. YRP_SQL_STR_IN(status) .. ", " .. YRP_SQL_STR_IN(name) .. "")
