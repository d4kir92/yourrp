...
function AddToHandler_Database(ply)
	if not table.HasValue(HANDLER_DATABASE, ply) then table.insert(HANDLER_DATABASE, ply) end
end
...
net.Receive("nws_yrp_connect_Settings_Database", function(len, ply)
	...
	for i, tab in pairs(tables) do
		if table.HasValue(GetDBNames(), tab.name) then
			if tab.name ~= "yrp_sql" then table.insert(nw_yrp, tab) end
...
function YRPCreateBackup()
	...
	if CreateYRPBackupsFolder() then
		...
		local svdb = file.Read("sv.db", "DATA")
		if svdb == nil then
			YRP:msg("db", "[YRPCreateBackup] Failed to Read DATABASE FILE (sv.db)")
			return false
		end
		...
		YRP:msg("db", "[BACKUP] Created Backup")
		return true
	else
		return false
	end
