...
hook.Add("YRP_SQLDBREADY_GENERAL", "yrp_MAPNAME_DB", function()
...
hook.Add("YRP_SQLDBREADY_GENERAL", "yrp_MAPNAME", function()
...
function YRPTeleportToSpawnpoint(ply, from)
	if ply.ignorespawnpoint == true then
		timer.Simple(2.0, function() if IsValid(ply) and ply.ignorespawnpoint then ply.ignorespawnpoint = false end end)
		return false
	else
		...
		if IsNotNilAndNotFalse(chaTab) and IsNotNilAndNotFalse(groTab) and IsNotNilAndNotFalse(rolTab) then
			...
			if IsNotNilAndNotFalse(_roleSpawnpoints) then
				...
				if ply:IsPlayer() then ply:SetEyeAngles(Angle(_tmp[1], _tmp[2], _tmp[3])) end
				return true
			elseif IsNotNilAndNotFalse(_groupSpawnpoints) then
...
net.Receive("nws_yrp_removeMapEntry", function(len, ply)
	...
	if _tmpMapTable ~= nil then
		...
		if _tmpMapTable.type == "dealer" then dealer_rem(_tmpMapTable.linkID) end
	end
...
net.Receive("nws_yrp_getMapList", function(len, ply)
	...
	if not IsNotNilAndNotFalse(_tmpMapTable) then _tmpMapTable = {} end
	local _tmpDealerTable = YRP_SQL_SELECT("yrp_dealers", "*", "map = '" .. GetMapNameDB() .. "'")
	if not IsNotNilAndNotFalse(_tmpDealerTable) then _tmpDealerTable = {} end
...
net.Receive("nws_yrp_dealer_settings", function(len, ply)
	...
	if _storages == nil or _storages == false then _storages = {} end
...
net.Receive("nws_yrp_getMapTab", function(len, ply)
	...
	local dbGrp = {}
	local dbRol = {}
	if grp then dbGrp = YRP_SQL_SELECT("yrp_ply_groups", "uniqueID, string_name", nil) end
	if rol then dbRol = YRP_SQL_SELECT("yrp_ply_roles", "uniqueID, int_groupID, string_name", nil) end
	if not IsNotNilAndNotFalse(dbTab) then dbTab = {} end
...
function YRPRegisterObject(obj)
	...
	if obj._suid == nil then
		...
		if IsNotNilAndNotFalse(storage) then obj._suid = tonumber(storage.uniqueID) end
		...
		if IsNotNilAndNotFalse(last) then last = last[1] end
	end
end
	...
	if IsNotNilAndNotFalse(storages) then
		for i, v in pairs(storages) do
			...
			for j, ent in pairs(ents.GetAll()) do
				if ent and ent._suid and ent._suid == v.linkID then found = true end
			end
...
hook.Add("EntityRemoved", "YRP_EntityRemoved", function(ent, fullUpdate) if not ent:IsSolid() and ent:GetMoveType() == MOVETYPE_NONE and ent:GetNoDraw() then YRPUnRegisterObject(ent) end end)
