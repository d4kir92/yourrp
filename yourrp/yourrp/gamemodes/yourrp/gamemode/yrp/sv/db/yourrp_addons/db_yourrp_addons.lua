...
function AddToHandler_YourRP_Addons(ply)
	if not table.HasValue(HANDLER_YOURRP_ADDONS, ply) then table.insert(HANDLER_YOURRP_ADDONS, ply) end
end
...
net.Receive("nws_yrp_connect_Settings_YourRP_Addons", function(len, ply)
	if ply:CanAccess("bool_yourrp_addons") then
		...
		if table.Count(yrp_addons) == 0 then hook.Run("get_yourrp_addons") end
...
net.Receive("nws_yrp_disconnect_Settings_YourRP_Addons", function(len, ply) RemFromHandler_YourRP_Addons(ply) end)
