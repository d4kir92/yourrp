--Copyright (C) 2017-2026 D4KiR (https://www.gnu.org/specializations/gpl.txt)
-- DO NOT TOUCH THE DATABASE FILES! If you have errors, report them here:
-- https://discord.gg/sEgNZxg
local DATABASE_NAME = "yrp_specializations"
hook.Add("YRP_SQLDBREADY_GAMEPLAY_DB", "yrp_specializations", function()
...
function YRPSendSpecPMs(uid, ply)
	...
	for i, v in pairs(tab) do
		if not table.HasValue(newtab, v) and not strEmpty(v) then table.insert(newtab, v) end
	end
...
net.Receive("nws_yrp_spec_add_pm", function(len, ply)
	...
	for i, pm in pairs(pms) do
		...
		if not table.HasValue(newtab, pm) then table.insert(newtab, pm) end
...
net.Receive("nws_yrp_spec_rem_pm", function(len, ply)
	...
	if table.HasValue(newtab, pm) then table.RemoveByValue(newtab, pm) end
	local newpms = table.concat(newtab, ",")
	YRP_SQL_UPDATE(DATABASE_NAME, {
		["pms"] = newpms
	}, "uniqueID = '" .. uid .. "'")
...
net.Receive("nws_yrp_spec_add_swep", function(len, ply)
	...
	for i, swep in pairs(sweps) do
		...
		if tab.sweps then
			...
			if not table.HasValue(newtab, swep) then table.insert(newtab, swep) end
...
net.Receive("nws_yrp_spec_rem_swep", function(len, ply)
	...
	if table.HasValue(newtab, swep) then table.RemoveByValue(newtab, swep) end
	local newsweps = table.concat(newtab, ",")
	YRP_SQL_UPDATE(DATABASE_NAME, {
		["sweps"] = newsweps
	}, "uniqueID = '" .. uid .. "'")
...
net.Receive("nws_yrp_get_specializations", function(len, ply) if ply:CanAccess("bool_specializations") then send_specializations(ply) end end)
function sendspecializations(ply)
	local _all = YRP_SQL_SELECT(DATABASE_NAME, "*", nil)
	local _nm = _all
	if _nm == nil or _nm == false then _nm = {} end
...
net.Receive("nws_yrp_getspecializations", function(len, ply) sendspecializations(ply) end)
...
net.Receive("nws_yrp_edit_specialization_name", function(len, ply)
	...
	if not _uid then return end
	local _edit = YRP_SQL_UPDATE(DATABASE_NAME, {
		["name"] = _new_name
	}, "uniqueID = " .. _uid)
...
net.Receive("nws_yrp_get_all_specializations_simple", function(len, ply)
	...
	if _all == false or _all == nil then _all = {} end
...
net.Receive("nws_yrp_role_add_specialization", function(len, ply)
	...
	if _role ~= nil then
		...
		if _role.specializationIDs ~= "" then _specializationIDs = string.Explode(",", _role.specializationIDs) end
...
function Player:AddSpecialization(specialization)
	...
	if tonumber(specialization) ~= nil then
		...
		if not table.HasValue(_specializationIDs, specialization) then table.insert(_specializationIDs, specialization) end
		if table.HasValue(_specializationIDs, "") then table.RemoveByValue(_specializationIDs, "") end
...
function Player:RemoveSpecialization(specialization)
	...
	if tonumber(specialization) ~= nil then
		local _specializationIDs = self:GetYRPString("specializationIDs", "")
		_specializationIDs = string.Explode(",", _specializationIDs)
		if table.HasValue(_specializationIDs, specialization) then table.RemoveByValue(_specializationIDs, specialization) end
		if table.HasValue(_specializationIDs, "") then table.RemoveByValue(_specializationIDs, "") end
		_specializationIDs = table.concat(_specializationIDs, ",")
		self:SetYRPString("specializationIDs", tostring(_specializationIDs))
		local ids = string.Explode(",", _specializationIDs)
...
function GetSpecializationIDByName(lname)
	...
	for i, spe in pairs(tab) do
		...
		if lname and spe.name and string.find(spe.name, lname, 1, true) then lid = spe.uniqueID end
	end
