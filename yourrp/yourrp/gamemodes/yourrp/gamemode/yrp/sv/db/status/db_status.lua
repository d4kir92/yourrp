...
function AddToHandler_Status(ply)
	if not table.HasValue(HANDLER_STATUS, ply) then table.insert(HANDLER_STATUS, ply) end
end
...
net.Receive("nws_yrp_connect_Settings_Status", function(len, ply)
	if ply:CanAccess("bool_status") then
		...
		for i, addon in pairs(engine.GetAddons()) do
			if addon.wsid == "1189643820" then _yourrp_content_found = true end
		end
		...
		if IsNotNilAndNotFalse(_map) and IsNotNilAndNotFalse(_groups) then
			for i, group in pairs(_groups) do
				...
				for j, entry in pairs(_map) do
					if entry.type == "GroupSpawnpoint" and tonumber(entry.linkID) == tonumber(group.uniqueID) then _has_no_spawnpoint = false end
				end
			...
			if _no_jailpoint then _nw_map["jail"]["LID_nojailpointfound"] = Color(220, 0, 0) end
			if _no_releasepoint then _nw_map["jail"]["LID_noreleasepointfound"] = Color(220, 0, 0) end
		end
...
net.Receive("nws_yrp_disconnect_Settings_Status", function(len, ply) RemFromHandler_Status(ply) end)
