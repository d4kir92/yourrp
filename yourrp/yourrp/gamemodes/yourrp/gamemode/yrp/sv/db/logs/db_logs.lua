...
hook.Add("YRP_SQLDBREADY_GENERAL_DB", "yrp_logs", function()
...
hook.Add("YRP_SQLDBREADY_GENERAL_UPDATE", "yrp_logs", function()
	local logTab = YRP_SQL_SELECT(DATABASE_NAME, "*", nil)
	if IsNotNilAndNotFalse(logTab) then
		for i, t in pairs(logTab) do
			if os.time() - deleteafter > tonumber(t.string_timestamp) then YRP_SQL_DELETE_FROM(DATABASE_NAME, "uniqueID = '" .. t.uniqueID .. "'") end
		end
	end
end)

YRP:AddNetworkString("nws_yrp_get_logs")
net.Receive("nws_yrp_get_logs", function(len, ply)
