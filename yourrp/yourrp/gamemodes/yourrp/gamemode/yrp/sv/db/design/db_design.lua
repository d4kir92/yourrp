...
hook.Add("YRP_SQLDBREADY_VISUAL_DB", "yrp_design", function()
	...
	if fir == nil then YRP_SQL_INSERT_INTO(DATABASE_NAME, "string_hud_design, string_interface_design, string_fontname", "'Simple', 'Material', 'Roboto'") end
end)

hook.Add("YRP_SQLDBREADY_VISUAL_UPDATE", "yrp_design", function()
	local fir = YRP_SQL_SELECT(DATABASE_NAME, "*", "uniqueID = 1")
...
hook.Add("YRP_SQLDBREADY_VISUAL", "yrp_design", function()
...
net.Receive("nws_yrp_ply_changed_resolution", function(len, ply)
	...
	timer.Simple(1, function() if IsValid(ply) then SetGlobalYRPInt("YRPHUDVersion", -2) end end)
	timer.Simple(2, function() if IsValid(ply) then SetGlobalYRPInt("YRPHUDVersion", -1) end end)
end)
...
net.Receive("nws_yrp_set_font", function(len, ply) YRPSendFontName(ply) end)
