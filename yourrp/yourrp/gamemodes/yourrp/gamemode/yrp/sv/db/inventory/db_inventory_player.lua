...
function Player:DropSWEP(cname, force)
	if (self:IsAllowedToDropSWEPRole(cname) and self:IsAllowedToDropSWEPUG(cname)) or force then
		...
		if self.dropdelay < CurTime() or force then
			...
			if IsValid(wep) then wep:SetYRPBool("canpickup", false) end
			...
			if ent and ent.WorldModel == "" then ent.WorldModel = "models/props_junk/garbage_takeoutcarton001a.mdl" end
			if IsValid(ent) then
				...
				timer.Simple(0.8, function() if IsValid(ent) then ent:SetYRPBool("canpickup", true) end end)
				timer.Simple(0.0, function()
					if IsValid(ent) and ent.Spawn then
						local _, err = pcall(YRPEntSpawn, ent)
						if err then YRPMsg(err) end
						if IsValid(wep) and wep.GetClip1 then ent:SetYRPInt("clip1", wep:GetClip1()) end
						if force == nil and ent:GetPhysicsObject():IsValid() and IsValid(self) then ent:GetPhysicsObject():SetVelocity(self:EyeAngles():Forward() * 360) end
						...
						timer.Simple(ttl, function()
							if YRPEntityAlive(ent) and ent.GetOwner and not YRPEntityAlive(ent:GetOwner()) then
								if ttl <= 1 then YRP:msg("note", "SWEP was removed TTL: " .. ttl) end
								ent:Remove()
							end
						end)
					end
