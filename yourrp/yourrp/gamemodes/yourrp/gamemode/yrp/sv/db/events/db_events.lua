...
hook.Add("YRP_SQLDBREADY_GAMEPLAY_DB", "yrp_events", function()
...
function YRPSendEvents(ply)
	...
	if not IsNotNilAndNotFalse(tab) then tab = {} end
...
net.Receive("nws_yrp_get_events", function(len, ply) if ply:CanAccess("bool_events") then YRPSendEvents(ply) end end)
		...
		if euid and euid ~= "" then
			...
			if IsNotNilAndNotFalse(tab) then
				...
				for i, v in pairs(tab) do
					...
					if test[2] ~= cuid then
						if newchars ~= "" then newchars = newchars .. ";" end
						newchars = newchars .. v
					end
				end
...
function YRPSpawnAsCharacter(ply, cuid, force)
	...
	if IsNotNilAndNotFalse(roltab) then YRPUpdateRoleUses(roltab.uniqueID) end
	ply:SetYRPBool("yrp_chararchived", false)
	if cuid ~= ply:CharID() then
		if GetGlobalYRPBool("bool_removebuildingownercharswitch", false) then YRPBuildingRemoveOwner(ply:YRPSteamID()) end
	...
	if IsNotNilAndNotFalse(cuid) then
		...
		timer.Simple(0.1, function() if YRPEntityAlive(ply) then ply:Spawn() end end)
	else
		...
		for i, charstring in pairs(chars) do
			...
			for n, p in pairs(player.GetAll()) do
				if p:YRPSteamID() == chartab[1] then
					...
					if plytab then YRPSpawnAsCharacter(p, plytab.NormalCharacter) end
				end
			end
		end
