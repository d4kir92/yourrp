...
hook.Add("YRP_SQLDBREADY_GAMEPLAY_DB", "yrp_realistic", function()
...
hook.Add("YRP_SQLDBREADY_GAMEPLAY", "yrp_realistic", function()
	local _init_realistic = YRP_SQL_SELECT(DATABASE_NAME, "*", nil)
	if _init_realistic ~= false and _init_realistic ~= nil then yrp_realistic = _init_realistic[1] end
...
function AddToHandler_Realistic(ply)
	if not table.HasValue(HANDLER_REALISTIC, ply) then table.insert(HANDLER_REALISTIC, ply) end
end
...
net.Receive("nws_yrp_disconnect_Settings_Realistic", function(len, ply) RemFromHandler_Realistic(ply) end)
