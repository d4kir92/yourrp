...
hook.Add("YRP_SQLDBREADY_GAMEPLAY_DB", "yrp_levelsystem", function()
	...
	if YRP_SQL_SELECT(DATABASE_NAME, "*", "uniqueID = 1") == nil then YRP_SQL_INSERT_INTO(DATABASE_NAME, "uniqueID", "'1'") end
end)

hook.Add("YRP_SQLDBREADY_GAMEPLAY", "yrp_levelsystem", function()
	local system = YRP_SQL_SELECT(DATABASE_NAME, "*", "uniqueID = '1'")
	if IsNotNilAndNotFalse(system) then xpPerMinute = tonumber(system[1].int_xp_per_minute) end
end)
