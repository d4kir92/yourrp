...
function PANEL:Paint(pw, ph)
	...
	if self.mdl:GetWide() ~= pw or self.mdl:GetTall() ~= ph then self.mdl:SetSize(pw, ph) end
	if self._world and not IsValid(self:GetE()) then self:Remove() end
end
...
net.Receive("nws_yrp_storage_open", function(len)
	...
	if sto ~= nil then
		...
		function bag:Paint(pw, ph)
			...
			if YRPInventory() == nil then self:Remove() end
		end
