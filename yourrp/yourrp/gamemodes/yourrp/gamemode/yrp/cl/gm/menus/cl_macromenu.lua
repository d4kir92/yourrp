...
function YRPToggleMacroMenu()
	if YRPIsNoMenuOpen() then YRPOpenMacroMenu() end
end
...
function YRPUseMacro(uid)
	if IsNotNilAndNotFalse(uid) and IsNotNilAndNotFalse(_mm.tab) and IsNotNilAndNotFalse(_mm.tab[uid]) and IsNotNilAndNotFalse(_mm.tab[uid].value) then
		...
		for i, v in pairs(tab) do
			if strEmpty(v) then table.RemoveByValue(tab, v) end
		end
		...
		for i, v in pairs(tab) do
			if not strEmpty(v) and not string.StartWith(v, "#") then
				if string.StartWith(v, "/timer") then
					...
					timer.Simple(tim, function() LocalPlayer():ConCommand("say \"" .. v .. "\"") end)
				end
			end
		end
	end
		...
		for y = 0, 6 do
			for x = 0, 6 do
				...
				function m:DoClick()
					...
					if keybind ~= nil then _mm.bind:SetValue(keybind) end
				end
