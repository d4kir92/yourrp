...
net.Receive("nws_yrp_setting_events", function(len)
	...
	if YRPPanelAlive(PARENT) then
		...
		net.Receive("nws_yrp_get_event_chars", function(le)
			...
			if tab.string_chars then
				...
				for n, chartab in pairs(ctab) do
					...
					for i, v in pairs(player.GetAll()) do
						if v:YRPSteamID() == chartab[1] then rpname = v:RPName() end
					end
		...
		function EVENT.AddChar:Paint(pw, ph)
			if EVENT.EventList and EVENT.EventList:GetSelectedLine() then hook.Run("YButtonAPaint", self, pw, ph) end
		end
		...
		function EVENT.RemoveChar:Paint(pw, ph)
			if EVENT.EventChars:GetSelectedLine() then hook.Run("YButtonRPaint", self, pw, ph) end
		end
		...
		function EVENT.RemoveEvent:Paint(pw, ph)
			if EVENT.EventList:GetSelectedLine() then hook.Run("YButtonRPaint", self, pw, ph) end
		end
		...
		function EVENT.StartEvent:Paint(pw, ph)
			if EVENT.EventList:GetSelectedLine() and not GetGlobalYRPBool("yrp_event_running", false) then hook.Run("YButtonAPaint", self, pw, ph) end
		end
		...
		function EVENT.EndEvent:Paint(pw, ph)
			if EVENT.EventList:GetSelectedLine() and GetGlobalYRPBool("yrp_event_running", false) then hook.Run("YButtonRPaint", self, pw, ph) end
		end
