	...
	if YRPPanelAlive(PARENT) then
		...
		function _li.eaf(tbl)
			...
			net.Receive("nws_yrp_get_specialization_sweps", function()
				...
				for i, v in pairs(tab_pm) do
					...
					swep.DoClick = function()
						...
						for id, w in pairs(string.Explode(",", tbl.sweps or "")) do
							if w ~= swep.string_classname then table.insert(tmp, w) end
						end
				...
				if _li.sweps.dpl.AddLines ~= nil then _li.sweps.dpl:AddLines(cl_sweps) end
			end)
			...
			net.Receive("nws_yrp_get_specialization_pms", function()
				...
				for i, v in pairs(tab_pm) do
					...
					pms2.DoClick = function()
						...
						for id, w in pairs(string.Explode(",", tbl.pms or "")) do
							if w ~= pms2.string_classname then table.insert(tmp, w) end
						end
				...
				if _li.pms.dpl.AddLines ~= nil then _li.pms.dpl:AddLines(cl_pms) end
			end)
