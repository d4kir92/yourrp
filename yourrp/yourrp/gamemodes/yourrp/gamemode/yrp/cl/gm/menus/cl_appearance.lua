...
net.Receive("nws_yrp_get_menu_bodygroups", function(len)
	...
	if _tbl.string_playermodels ~= nil and YRPPanelAlive(_yrp_appearance.window, "_yrp_appearance.window") then
		...
		if YRPPanelAlive(_yrp_appearance.left, "_yrp_appearance.left") then
			...
			if _pmid > #_pms then _pmid = 1 end
			local _pm = _pms[_pmid]
			if _pm == "" or _pm == " " then _pm = "models/player/skeleton.mdl" end
			if pm ~= "" then
				local _cbg = {}
				...
				function _tmpPMUp:Paint(pw, ph)
					if _tmpPM.cur < _tmpPM.max then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
				end

				function _tmpPMUp:DoClick()
					if _tmpPM.cur < _tmpPM.max then _tmpPM.cur = _tmpPM.cur + 1 end
					net.Start("nws_yrp_inv_pm_up")
					net.WriteInt(_tmpPM.cur, 16)
					net.SendToServer()
					if _appe.r.pm.Entity then _appe.r.pm.Entity:SetModel(_pms[_tmpPM.cur]) end
				end
				...
				function _tmpPMDo:Paint(pw, ph)
					if _tmpPM.cur > 1 then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
				end

				function _tmpPMDo:DoClick()
					if _tmpPM.cur > 1 then _tmpPM.cur = _tmpPM.cur - 1 end
				...
				-- Skin changing
				if _appe.r.pm.Entity then _tbl.bgs = _appe.r.pm.Entity:GetBodyGroups() end
				...
				function _tmpSkinUp:Paint(pw, ph)
					if _tmpSkin.cur < _tmpSkin.max - 1 then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
				end

				function _tmpSkinUp:DoClick()
					if _tmpSkin.cur < _tmpSkin.max - 1 then _tmpSkin.cur = _tmpSkin.cur + 1 end
					net.Start("nws_yrp_inv_skin_up")
					net.WriteInt(_tmpSkin.cur, 16)
					net.SendToServer()
					if _appe.r.pm.Entity then _appe.r.pm.Entity:SetSkin(_tmpSkin.cur) end
				end
				...
				function _tmpSkinDo:Paint(pw, ph)
					if _tmpSkin.cur > 0 then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
				end

				function _tmpSkinDo:DoClick()
					if _tmpSkin.cur > 0 then _tmpSkin.cur = _tmpSkin.cur - 1 end
					net.Start("nws_yrp_inv_skin_do")
					net.WriteInt(_tmpSkin.cur, 16)
					net.SendToServer()
					if YRPEntityAlive(_appe.r.pm.Entity) then _appe.r.pm.Entity:SetSkin(_tmpSkin.cur) end
				end
				...
				if _tbl.bgs then
					for k, v in pairs(_tbl.bgs) do
						if _cbg[k] ~= nil then
							...
							if _appe.r.pm.Entity then _appe.r.pm.Entity:SetBodygroup(k - 1, tonumber(_cbg[k])) end
							local _height = 80
							...
							function _tmpBgUp:Paint(pw, ph)
								if _tmpBg.cur < _tmpBg.max - 1 then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
							end

							function _tmpBgUp:DoClick()
								if _tmpBg.cur < _tmpBg.max - 1 then _tmpBg.cur = _tmpBg.cur + 1 end
								net.Start("nws_yrp_inv_bg_up")
								net.WriteInt(_tmpBg.cur, 16)
								net.WriteInt(_tmpBg.id, 16)
								net.SendToServer()
								if _appe.r.pm.Entity then _appe.r.pm.Entity:SetBodygroup(_tmpBg.id, _tmpBg.cur) end
							end
							...
							function _tmpBgDo:Paint(pw, ph)
								if _tmpBg.cur > 0 then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
							end

							function _tmpBgDo:DoClick()
								if _tmpBg.cur > 0 then _tmpBg.cur = _tmpBg.cur - 1 end
...
function YRPToggleAppearanceMenu()
	if YRPIsNoMenuOpen() then
		if GetGlobalYRPBool("bool_appearance_system", false) then open_appearance() end
	else
		close_appearance()
	end
end
...
function close_appearance()
	if _yrp_appearance.window ~= nil then
		...
		if _yrp_appearance.drop_panel ~= nil then _yrp_appearance.drop_panel:Remove() end
		_yrp_appearance.window = nil
	end
end

net.Receive("nws_yrp_openAM", function(len) open_appearance() end)
function open_appearance()
	...
	function _yrp_appearance.window:OnClose()
		if YRPPanelAlive(_yrp_appearance.window, "_yrp_appearance.window 4") then _yrp_appearance.window:Remove() end
	end
