...
function getCopyMapPNG()
	...
	if file.Exists(_mapPicturePath, "GAME") then
		if not file.Exists("maps", "DATA") then file.CreateDir("maps") end
		file.Write(_mapPicturePath, file.Read(_mapPicturePath, "GAME"))
		if file.Exists(_mapPicturePath, "DATA") then _mapPNG = Material("../data/" .. _mapPicturePath, "noclamp smooth") end
	else
		_mapPicturePath = "maps/thumb/" .. _mapName .. ".png"
		if file.Exists(_mapPicturePath, "GAME") then
			if not file.Exists("maps", "DATA") then file.CreateDir("maps") end
			file.Write(_mapPictureDesti, file.Read(_mapPicturePath, "GAME"))
			if file.Exists(_mapPictureDesti, "DATA") then _mapPNG = Material("../data/" .. _mapPictureDesti, "noclamp smooth") end
		end
	end
	return _mapPNG
end
...
net.Receive("nws_yrp_getMapSite", function(len)
	...
	if YRPPanelAlive(PARENT) then
		if len > 512000 then YRP:msg("note", "getMapList - len: " .. len .. "/" .. "512000 (len is to big)") end
		local tabs = YRPCreateD("YTabs", PARENT, PARENT:GetWide(), PARENT:GetTall(), 0, 0)
		tabs:SetTabWide(540)
		function tabs:Think()
			if YRPPanelAlive(PARENT) then self:SetSize(PARENT:GetWide(), PARENT:GetTall()) end
		end
...
net.Receive("nws_yrp_getMapTab", function(len)
		...
		if tab == "groupspawnpoints" or tab == "rolespawnpoints" or tab == "dealers" or tab == "storagepoints" then
			...
			if tab == "groupspawnpoints" or tab == "rolespawnpoints" then
				...
				function addWin.addGrp:OnSelect(index, value, data)
					if tab == "groupspawnpoints" then addWin.linkID = data end
					if addWin.addRol then
						addWin.addRol:Clear()
						for k, r in pairs(dbRol) do
							if r.int_groupID == data then addWin.addRol:AddChoice(r.string_name, r.uniqueID) end
						end
					end
				end
			...
			if tab == "rolespawnpoints" then
				...
				function addWin.addRol:OnSelect(index, value, data)
					if tab == "rolespawnpoints" then addWin.linkID = data end
				end
	...
	function btnAdd:Paint(pw, ph)
		if tab == "groupspawnpoints" or tab == "rolespawnpoints" or tab == "dealers" or tab == "storagepoints" then hook.Run("YButtonPaint", self, pw, ph) end
	end
