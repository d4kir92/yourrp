	...
	if YRPPanelAlive(PARENT) then
		...
		function _giveListView:OnRowRightClick(lineID, line)
			...
			function _buttonGiveLicense:DoClick()
				...
				net.Receive("nws_yrp_get_licenses_player", function(le)
			...
			function _tmpPanel:Paint(pw, ph)
				...
				if not _tmpPanel:IsHovered() and not _buttonSetRPName:IsHovered() and not _buttonRole:IsHovered() and not _buttonSetID:IsHovered() and not _buttonRemoveArrests:IsHovered() and not _buttonGiveLicense:IsHovered() and not _buttonRemoveLicense:IsHovered() and _tmpPanel.ready == true then _tmpPanel:Remove() end
			end
