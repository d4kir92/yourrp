...
net.Receive("nws_yrp_get_shops", function()
	...
	if YRPPanelAlive(PARENT) then
		...
		function _sh._sho:RemoveFunction()
			...
			net.SendToServer()
			if _sh._cat ~= nil then _sh._cat:Remove() end
			if _sh._sit ~= nil then _sh._sit:Remove() end
		end
				...
				net.Receive("nws_yrp_get_all_licenses_simple", function(len)
					...
					if YRPPanelAlive(_sh._sit) and YRPPanelAlive(_sh._sit.itemlice) and YRPPanelAlive(_sh._sit.itemlice.plus) then
						...
						for i, lic in pairs(_licenses) do
							local _b = false
							if tonumber(lic.uniqueID) == tonumber(tbl.licenseID) then _b = true end
