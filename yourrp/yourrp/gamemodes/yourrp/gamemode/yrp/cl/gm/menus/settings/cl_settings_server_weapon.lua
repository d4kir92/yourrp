...
net.Receive("nws_yrp_weapon_menu_weapon", function(len)
	...
	cou = cou + 1
	local c = cou
	timer.Simple(0.1, function() if c == cou then YRPWeaponUpdateList() end end)
end)
		...
		for i, swep in SortedPairsByMemberValue(cl_sweps, "PrintName") do
			...
			if string.find(string.lower(swep.PrintName), searcstr, 1, true) or string.find(string.lower(swep.ClassName), searcstr, 1, true) then
				...
				function weapon:Paint(pw, ph)
					...
					if DBTab2[swep.ClassName].slot_no == 1 then color = Color(20, 20, 20) end
					if YRPWeaponNoEntry(swep.ClassName) then color = Color(160, 0, 0) end
					draw.RoundedBox(3, 0, 0, pw, ph, color)
					local text = swep.PrintName
					if strEmpty(text) then text = "ClassName: " .. swep.ClassName end
					if string.find(string.lower(swep.ClassName), "base", 1, true) or string.find(string.lower(swep.PrintName), "base", 1, true) then text = text .. " [MAYBE A BASE]" end
