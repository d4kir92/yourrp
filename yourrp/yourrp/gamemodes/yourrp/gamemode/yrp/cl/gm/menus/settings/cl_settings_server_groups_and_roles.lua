...
net.Receive("nws_yrp_subscribe_Settings_GroupsAndRoles", function(len)
	...
	if YRPPanelAlive(PARENT, "GAR 1") then
		...
		net.Receive("nws_yrp_settings_group_update_name", function(le)
			...
			if YRPPanelAlive(gs.gplist, "GAR 2") and gs.gplist[_uid] and gs.gplist[_uid].text then gs.gplist[_uid].text = name end
		end)

		net.Receive("nws_yrp_settings_group_update_color", function(le)
			local _uid = tonumber(net.ReadString())
			local color = net.ReadString()
			if IsNotNilAndNotFalse(gs) and gs.gplist and gs.gplist[_uid] and gs.gplist[_uid].string_color and isnumber(_uid) and gs.gplist[_uid] ~= nil then gs.gplist[_uid].string_color = stc(color) end
		end)
		...
		function rs.add:Paint(pw, ph)
			if rs.top.headername ~= nil then hook.Run("YButtonAPaint", self, pw, ph) end
		end
		...
		net.Receive("nws_yrp_settings_role_update_color", function(le)
			if IsNotNilAndNotFalse(rs) then
				local _uid = tonumber(net.ReadString())
				local color = net.ReadString()
				if isnumber(_uid) and rs.rplist[_uid] ~= nil then rs.rplist[_uid].string_color = stc(color) end
			end
		end)
		...
		function ea.del:DoClick()
			if ea.typ == "group" and tonumber(ea.tab.uniqueID) ~= 1 then
				...
				function _yes:DoClick()
					...
					if ea and YRPEntityAlive(ea.background) and ea.background.Clear then ea.background:Clear() end
					if ea then
						ea.typ = nil
						ea.tab = nil
					end

					if win and win.Close then win:Close() end
				end
		...
		function ea.top:Paint(pw, ph)
			if ea.typ ~= nil then
				...
				if ea.tab.string_identifier and not strEmpty(ea.tab.string_identifier) then darkrpjobname = ea.tab.string_identifier end
			...
			if YRPPanelAlive(ea.background, "GAR 8") then
				...
				if group.uniqueID and group.uniqueID > 0 then
					...
					sweps.DoClick = function()
						...
						function winswep:Search(strsearch)
							...
							for i, v in pairs(cl_sweps) do
								if (v.PrintName and type(v.PrintName) == "string" and string.find(string.lower(v.PrintName), strsearch, 1, true)) or (v.ClassName and type(v.ClassName) == "string" and string.find(string.lower(v.ClassName), strsearch, 1, true)) or (v.WorldModel and type(v.WorldModel) == "string" and string.find(string.lower(v.WorldModel), strsearch, 1, true)) then
									...
									function d_swep:DoClick()
										...
										net.SendToServer()
										if winswep and winswep.Close then winswep:Close() end
									end
					...
					net.Receive("nws_yrp_settings_get_sweps", function()
						...
						if ea[group.uniqueID].sweps.dpl.AddLines ~= nil then ea[group.uniqueID].sweps.dpl:AddLines(cl_sweps) end
					end)
		...
		net.Receive("nws_yrp_settings_subscribe_role", function(le)
			...
			if role.uniqueID > 1 then
				...
				for i, tab in pairs(db_groups) do
					tab.uniqueID = tonumber(tab.uniqueID)
					if IsNotNilAndNotFalse(tab.string_name) and IsNotNilAndNotFalse(tab.uniqueID) and tab.uniqueID ~= -1 then grps[tab.uniqueID] = tab.string_name .. " [UID: " .. tab.uniqueID .. "]" end
				end
			...
			net.Receive("nws_yrp_get_role_customflags", function()
				...
				if tab_cf then
					...
					if ea[role.uniqueID].custom_flags.dpl.AddLines ~= nil then ea[role.uniqueID].custom_flags.dpl:AddLines(tab_cf) end
				end
			end)
			...
			playermodels.DoClick = function()
				...
				function win.add:DoClick()
					...
					function pmwin.float_min:OnValueChange(val)
						...
						if isnumber(val) and isnumber(maxval) and val > maxval then pmwin.float_max:SetValue(val) end
					end

					function pmwin.float_max:OnValueChange(val)
						val = tonumber(val)
						minval = tonumber(pmwin.float_min:GetValue())
						if isnumber(val) and isnumber(minval) and val < minval then pmwin.float_min:SetValue(val) end
					end
					...
					function pmwin.add:DoClick()
						if pmwin.WorldModel ~= "" then
							...
							net.WriteString(max)
							net.SendToServer()
							if pmwin and pmwin.Close then pmwin:Close() end
						end
					end
				end
				...
				net.Receive("nws_yrp_get_all_playermodels", function(l)
					if l > 128000 then YRP:msg("note", "[get_all_playermodels] len: " .. l) end
					local tab = net.ReadTable()
					table.insert(pms, tab)
					if YRPPanelAlive(win, "GAR 13") then win:Search(win:GetText()) end
				end)
			...
			net.Receive("nws_yrp_get_role_playermodels", function()
				...
				if ea[role.uniqueID].playermodels.dpl.AddLines ~= nil then ea[role.uniqueID].playermodels.dpl:AddLines(tab_pm) end
			end)
			...
			net.Receive("nws_yrp_get_role_sweps", function()
				...
				for i, v in pairs(tab_pm) do
					...
					swep.DoClick = function()
						...
						for id, va in pairs(string.Explode(",", role.string_sweps)) do
							if va ~= swep.string_classname then table.insert(tmp, va) end
						end
				...
				if ea[role.uniqueID].sweps.dpl.AddLines ~= nil then ea[role.uniqueID].sweps.dpl:AddLines(cl_sweps) end
			end)
			...
			if GetGlobalYRPBool("bool_weapon_system", true) then
				...
				net.Receive("nws_yrp_get_role_sweps_onspawn", function()
					...
					if ea[role.uniqueID].swepsonspawn.dpl.AddLines ~= nil then ea[role.uniqueID].swepsonspawn.dpl:AddLines(cl_sweps) end
				end)
				...
				net.Receive("nws_yrp_get_all_licenses", function(l)
					...
					if winlicenses then
								...
								for i, v in pairs(cl_licenses) do
									...
									if string.find(string.lower(v.PrintName), strsearch, 1, true) or string.find(string.lower(v.ClassName), strsearch, 1, true) or string.find(string.lower(v.WorldModel), strsearch, 1, true) then
										...
										function d_licenses:DoClick()
											...
											net.WriteString(v.ClassName)
											net.SendToServer()
											if winlicenses and winlicenses.Close then winlicenses:Close() end
										end
						...
						function winlicenses.search:OnChange()
							...
							if searchtext ~= nil then winlicenses:Search(searchtext) end
						end
					end
				end)
			...
			net.Receive("nws_yrp_get_role_licenses", function()
				...
				if ea[role.uniqueID].licenses.dpl.AddLines ~= nil then ea[role.uniqueID].licenses.dpl:AddLines(cl_licenses) end
			end)
					...
					for i, v in pairs(cl_ndsweps) do
						if string.find(string.lower(v.PrintName), strsearch, 1, true) or string.find(string.lower(v.ClassName), strsearch, 1, true) or string.find(string.lower(v.WorldModel), strsearch, 1, true) then
							...
							function d_ndswep:DoClick()
								...
								net.WriteString(v.ClassName)
								net.SendToServer()
								if winndswep and winndswep.Close then winndswep:Close() end
							end
							...
							if winndswep and winndswep.dpl then winndswep.dpl:AddItem(d_ndswep) end
						end
					end
				end
			...
			net.Receive("nws_yrp_get_role_ndsweps", function()
				...
				if cl_ndsweps and ea[role.uniqueID].ndsweps.dpl.AddLines ~= nil then ea[role.uniqueID].ndsweps.dpl:AddLines(cl_ndsweps) end
			end)
			...
			specializations.DoClick = function()
				...
				net.Receive("nws_yrp_get_all_specializations", function(l)
							...
							for i, v in pairs(cl_specializations) do
								...
								if string.find(string.lower(v.PrintName), strsearch, 1, true) or string.find(string.lower(v.ClassName), strsearch, 1, true) or string.find(string.lower(v.WorldModel), strsearch, 1, true) then
									...
									function d_specializations:DoClick()
										...
										net.WriteString(v.ClassName)
										net.SendToServer()
										if winspecializations and winspecializations.Close then winspecializations:Close() end
									end
					...
					function winspecializations.search:OnChange()
						local searchtext = self:GetText()
						if searchtext ~= nil then winspecializations:Search(searchtext) end
					end
				end)
			...
			net.Receive("nws_yrp_get_role_specializations", function()
				...
				if ea[role.uniqueID].specializations.dpl.AddLines ~= nil then ea[role.uniqueID].specializations.dpl:AddLines(cl_specializations) end
			end)
