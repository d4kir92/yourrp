...
net.Receive("nws_yrp_getCharakterList", function()
	...
	if YRPPanelAlive(PARENT) then
		...
		if _charTab.rpname ~= nil then cl_rpName:SetText(_charTab.rpname) end
		...
		if _charTab.rpdescription ~= nil then cl_rpDescription:SetText(_charTab.rpdescription) end
...
hook.Add("open_client_character", "open_client_character", function() if YRPPanelAlive(settingsWindow) then SaveLastSite() end end)
