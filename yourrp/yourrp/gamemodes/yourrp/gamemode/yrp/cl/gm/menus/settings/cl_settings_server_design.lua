...
net.Receive("nws_yrp_get_design_settings", function(len)
	...
	if YRPPanelAlive(PARENT) then
		...
		for i, design in pairs(HUDS) do
			...
			if design.name == setting.string_hud_design then selected = true end
			local name = design.name .. " by " .. design.author
			if tonumber(design.progress) < 100 then name = name .. " ( " .. design.progress .. "% " .. YRP:trans("LID_done") .. " )" end
		...
		function Customize()
			...
			function editarea:Paint(pw, ph)
				...
				if table.Count(editarea["settingswindows"]) == 0 then
					...
					for x = 0, ScW(), boxspace do
						...
						-- X L
						if count % self.hl == 0 and x < ScW() / 2 then color = Color(0, 0, 0, 160) end
						-- X R
						if count % self.hl == ScW() / boxspace % self.hl and x > ScW() / 2 then color = Color(0, 0, 0, 160) end
						-- X C
						if count % self.hl == ScW() / 2 / boxspace % self.hl then color = Color(0, 255, 0, 255) end
						-- X Center
						if ScW() / 2 == x then color = Color(0, 0, 255, 255) end
						color.a = cur_alpha
					...
					for y = 0, ScH(), boxspace do
						local color = Color(255, 255, 255, 160)
						-- Y U
						if count % self.hl == 0 and y < ScH() / 2 then color = Color(0, 0, 0, 160) end
						-- Y D
						if count % self.hl == ScH() / boxspace % self.hl and y > ScH() / 2 then color = Color(0, 0, 0, 160) end
						-- Y C
						if count % self.hl == ScH() / 2 / boxspace % self.hl then color = Color(0, 255, 0, 255) end
						-- Y Center
						if ScH() / 2 == y then color = Color(0, 0, 255, 255) end
						color.a = cur_alpha
			...
			function AddElement(tab)
				...
				function win.setting:DoClick()
					net.Receive("nws_yrp_get_hud_element_settings", function(le)
						if win and IsValid(win) then
							...
							function win.winset:Paint(pw, ph)
								...
								if input.IsMouseDown(MOUSE_FIRST) and (mx < x or mx > x + pw or my < y or my > y + ph) and YRPPanelAlive(win.winset.dpl) then
									...
									for i, item in pairs(childrens) do
										if item.cb ~= nil and item.cb:IsMenuOpen() then open = true end
									end

									if not open then win.winset:Close() end
								end
							...
							function win.winset:AddTextPosition(t)
								...
								function btn:Paint(pw, ph)
									...
									if self:IsHovered() then color = Color(255, 255, 0, 255) end
							...
							function win.winset:AddComboBox(t)
								...
								for i, choice in pairs(t.choices) do
									local selected = false
									if choice == t.value then selected = true end
							...
							function win.winset:AddColorMixer(t)
								...
								function btn:DoClick()
									...
									function cmwin:Paint(pw, ph)
										local x, y = self:GetPos()
										local mx, my = gui.MousePos()
										if input.IsMouseDown(MOUSE_FIRST) and (mx < x or mx > x + pw or my < y or my > y + ph) then cmwin:Close() end
									end
				...
				function win.setting:Paint(pw, ph)
					if win.visible then
						...
						if self:IsHovered() then color = Color(255, 255, 0, cur_alpha) end
		...
		for i, size in pairs(gridsizes) do
			...
			function hud_x:Paint(pw, ph)
				local color = Color(255, 255, 255, 255)
				if self:IsHovered() then color = Color(255, 255, 100) end
				draw.RoundedBox(0, 0, 0, pw, ph, color)
		...
		for i, v in pairs(hud_profiles) do
			local selected = false
			if GetGlobalYRPString("string_hud_profile", "") == v.value then selected = true end
		...
		for i, fo in pairs(YRP:GetFonts()) do
			local selected = false
			if fo.name == YRP:GetFont() then selected = true end
			local name = fo.name
