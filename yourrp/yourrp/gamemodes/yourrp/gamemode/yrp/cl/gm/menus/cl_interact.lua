...
function YRPToggleInteractMenu()
	...
	if eyeTrace.Entity:IsPlayer() and YRPIsNoMenuOpen() then
		if eyeTrace.Entity:GetColor().a > 0 then openInteractMenu(eyeTrace.Entity:CharID()) end
	else
		closeInteractMenu()
	end
end
...
net.Receive("nws_yrp_openInteractMenu", function(len)
	...
	for k, v in pairs(player.GetAll()) do
		if tostring(v:CharID()) == tostring(tmpTargetCharID) then
			...
			tmpRPDescription = ""
			for i = 1, 10 do
				if i > 1 then tmpRPDescription = tmpRPDescription .. "\n" end
	...
	if isInstructor then
		if promoteable then
			...
			function btnPromote:DoClick()
				...
				net.WriteString(tmpTargetCharID)
				net.SendToServer()
				if YRPPanelAlive(yrp_Interact, "yrp_Interact 1") then yrp_Interact:Close() end
			end
		...
		if demoteable then
			...
			function btnDemote:DoClick()
				...
				net.WriteString(tmpTargetCharID)
				net.SendToServer()
				if YRPPanelAlive(yrp_Interact, "yrp_Interact 2") then yrp_Interact:Close() end
			end
		...
		if not promoteable and not demoteable then
			...
			function btnbtnInviteToGroup:DoClick()
				...
				net.WriteString(tmpTargetCharID)
				net.SendToServer()
				if YRPPanelAlive(yrp_Interact, "yrp_Interact 3") then yrp_Interact:Close() end
			end
	...
	if hasspecs then
		...
		function btnbtnSpecialization:DoClick()
			if YRPPanelAlive(yrp_Interact, "yrp_Interact 4") then yrp_Interact:Close() end
