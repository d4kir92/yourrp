...
net.Receive("nws_yrp_connect_Settings_UserGroup", function(len)
	...
	if YRPPanelAlive(PARENT.ug) then PARENT.ug:Remove() end
	...
	net.Receive("nws_yrp_usergroup_update_string_name", function(len2)
		...
		if UGS and UGS[CURRENT_USERGROUP] then
			...
			if YRPPanelAlive(NAME) then NAME:SetText(string.upper(UGS[CURRENT_USERGROUP].string_name)) end
		end
	end)

	if tonumber(ug.bool_removeable) == 0 then NAME.plus:SetDisabled(true) end
	-- DISPLAYNAME
	...
	net.Receive("nws_yrp_usergroup_update_string_displayname", function(len2)
		...
		if UGS and UGS[CURRENT_USERGROUP] then
			...
			if IsValid(DISPLAYNAME) then DISPLAYNAME:SetText(UGS[CURRENT_USERGROUP].string_displayname) end
		end
	end)
	...
	function COLOR.plus:Paint(pw, ph)
		if IsNotNilAndNotFalse(UGS[CURRENT_USERGROUP]) then hook.Run("YButtonPaint", self, pw, ph) end
	end
	...
	function COLOR.plus:DoClick()
		...
		function window:Paint(pw, ph)
			...
			if not YRPPanelAlive(PARENT) then self:Remove() end
		end
	...
	net.Receive("nws_yrp_usergroup_update_string_color", function(len2)
		...
		if UGS and UGS[CURRENT_USERGROUP] then UGS[CURRENT_USERGROUP].string_color = color end
	end)
	...
	-- SWEPS
	if type(ug.string_sweps) == "string" then ug.string_sweps = string.Explode(",", ug.string_sweps) end
	local tmp = {}
	for i, v in pairs(ug.string_sweps) do
		if v ~= nil and not strEmpty(v) then table.insert(tmp, v) end
	end
	...
	function SWEPS.preview:PaintOver(pw, ph)
		if IsNotNilAndNotFalse(UGS[CURRENT_USERGROUP]) then
			if self.oldcur ~= self.cur then
				...
				if UGS[CURRENT_USERGROUP].string_sweps then self:SetModel(GetSWEPWorldModel(UGS[CURRENT_USERGROUP].string_sweps[self.cur])) end
			end

			surfaceText(self.cur .. "/" .. self.max, "Y_18_500", pw / 2, ph - YRP:ctr(30), Color(255, 255, 255, 255), 1, 1)
			if UGS[CURRENT_USERGROUP].string_sweps then surfaceText(UGS[CURRENT_USERGROUP].string_sweps[self.cur] or "NOMODEL", "Y_18_500", pw / 2, ph - YRP:ctr(70), Color(255, 255, 255, 255), 1, 1) end
		end
	end
	...
	function SWEPS.preview.prev:Paint(pw, ph)
		if SWEPS.preview.cur > 1 then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
	end

	function SWEPS.preview.prev:DoClick()
		if SWEPS.preview.cur > 1 then SWEPS.preview.cur = SWEPS.preview.cur - 1 end
	end
	...
	function SWEPS.preview.next:Paint(pw, ph)
		if SWEPS.preview.cur < SWEPS.preview.max then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
	end

	function SWEPS.preview.next:DoClick()
		if SWEPS.preview.cur < SWEPS.preview.max then SWEPS.preview.cur = SWEPS.preview.cur + 1 end
	end

	if type(UGS[CURRENT_USERGROUP].string_sweps) == "string" then UGS[CURRENT_USERGROUP].string_sweps = string.Explode(",", UGS[CURRENT_USERGROUP].string_sweps) end
	...
	function SWEPS.button:DoClick()
		...
		if UGS[CURRENT_USERGROUP].string_sweps then
			for i, v in pairs(UGS[CURRENT_USERGROUP].string_sweps) do
				if not table.HasValue(lply.yrpseltab) then table.insert(lply.yrpseltab, v) end
			end
		end
	...
	net.Receive("nws_yrp_usergroup_update_string_sweps", function(len2)
		if YRPPanelAlive(SWEPS) then
			...
			if type(UGS[CURRENT_USERGROUP].string_sweps) == "string" then UGS[CURRENT_USERGROUP].string_sweps = string.Explode(",", string_sweps) end
			local tmp4 = {}
			for i, v in pairs(ug.string_sweps) do
				if v ~= nil and not strEmpty(v) then table.insert(tmp4, v) end
			end
			...
			if UGS[CURRENT_USERGROUP].string_sweps[1] ~= "" then
				...
				if UGS[CURRENT_USERGROUP].string_sweps then SWEPS.preview:SetModel(GetSWEPWorldModel(UGS[CURRENT_USERGROUP].string_sweps[1] or "")) end
			else
				SWEPS.preview.cur = 0
				SWEPS.preview.max = 0
	...
	-- NONEDROPPABLESWEPS
	if type(ug.string_nonesweps) == "string" then ug.string_nonesweps = string.Explode(",", ug.string_nonesweps) end
	local tmp2 = {}
	for i, v in pairs(ug.string_nonesweps) do
		if v ~= nil and not strEmpty(v) then table.insert(tmp2, v) end
	end
	...
	function NONESWEPS.preview:PaintOver(pw, ph)
		if IsNotNilAndNotFalse(UGS[CURRENT_USERGROUP]) then
			if self.oldcur ~= self.cur then
				self.oldcur = self.cur
				if UGS[CURRENT_USERGROUP].string_nonesweps then self:SetModel(GetSWEPWorldModel(UGS[CURRENT_USERGROUP].string_nonesweps[self.cur])) end
			end

			surfaceText(self.cur .. "/" .. self.max, "Y_18_500", pw / 2, ph - YRP:ctr(30), Color(255, 255, 255, 255), 1, 1)
			if UGS[CURRENT_USERGROUP].string_nonesweps then surfaceText(UGS[CURRENT_USERGROUP].string_nonesweps[self.cur] or "NOMODEL", "Y_18_500", pw / 2, ph - YRP:ctr(70), Color(255, 255, 255, 255), 1, 1) end
		end
	end
	...
	function NONESWEPS.preview.prev:Paint(pw, ph)
		if NONESWEPS.preview.cur > 1 then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
	end

	function NONESWEPS.preview.prev:DoClick()
		if NONESWEPS.preview.cur > 1 then NONESWEPS.preview.cur = NONESWEPS.preview.cur - 1 end
	end
	...
	function NONESWEPS.preview.next:Paint(pw, ph)
		if NONESWEPS.preview.cur < NONESWEPS.preview.max then hook.Run("YButtonPaint", self, pw, ph, self.tab) end
	end

	function NONESWEPS.preview.next:DoClick()
		if NONESWEPS.preview.cur < NONESWEPS.preview.max then NONESWEPS.preview.cur = NONESWEPS.preview.cur + 1 end
	end
	...
	function NONESWEPS.button:DoClick()
		...
		if UGS[CURRENT_USERGROUP] and UGS[CURRENT_USERGROUP].string_nonesweps then
			for i, v in pairs(UGS[CURRENT_USERGROUP].string_nonesweps) do
				if not table.HasValue(lply.yrpseltab) and validate[v] then table.insert(lply.yrpseltab, v) end
			end
		end
	...
	net.Receive("nws_yrp_usergroup_update_string_nonesweps", function(len2)
		if YRPPanelAlive(NONESWEPS) then
			...
			if type(UGS[CURRENT_USERGROUP].string_nonesweps) == "string" then UGS[CURRENT_USERGROUP].string_nonesweps = string.Explode(",", string_nonesweps) end
			local tmp3 = {}
			for i, v in pairs(ug.string_nonesweps) do
				if v ~= nil and not strEmpty(v) then table.insert(tmp3, v) end
			end
			...
			if UGS[CURRENT_USERGROUP].string_nonesweps[1] ~= "" then
				...
				if UGS[CURRENT_USERGROUP].string_nonesweps then NONESWEPS.preview:SetModel(GetSWEPWorldModel(UGS[CURRENT_USERGROUP].string_nonesweps[1] or "")) end
			else
				NONESWEPS.preview.cur = 0
				NONESWEPS.preview.max = 0
	...
	if type(UGS[CURRENT_USERGROUP].string_licenses) == "string" then UGS[CURRENT_USERGROUP].string_licenses = string.Explode(",", UGS[CURRENT_USERGROUP].string_licenses) end
	if table.HasValue(UGS[CURRENT_USERGROUP].string_licenses, "") then table.RemoveByValue(UGS[CURRENT_USERGROUP].string_licenses, "") end
	net.Receive("nws_yrp_get_usergroup_licenses", function()
		...
		for i, lic in pairs(licenses) do
			...
			if type(UGS[CURRENT_USERGROUP].string_licenses) == "string" then UGS[CURRENT_USERGROUP].string_licenses = string.Explode(",", UGS[CURRENT_USERGROUP].string_licenses) end
			if CURRENT_USERGROUP and UGS[CURRENT_USERGROUP] and table.HasValue(UGS[CURRENT_USERGROUP].string_licenses, lic.uniqueID) then cb:SetChecked(true) end
			function cb:OnChange(bVal)
				if CURRENT_USERGROUP and UGS[CURRENT_USERGROUP] and UGS[CURRENT_USERGROUP].string_licenses and type(UGS[CURRENT_USERGROUP].string_licenses) == "string" then UGS[CURRENT_USERGROUP].string_licenses = string.Explode(",", UGS[CURRENT_USERGROUP].string_licenses) end
	...
	if tonumber(ug.bool_removeable) == 1 then
		...
		if type(UGS[CURRENT_USERGROUP].string_tools) == "string" then UGS[CURRENT_USERGROUP].string_tools = string.Explode(",", UGS[CURRENT_USERGROUP].string_tools) end
		if table.HasValue(UGS[CURRENT_USERGROUP].string_tools, "") then table.RemoveByValue(UGS[CURRENT_USERGROUP].string_tools, "") end
		...
		if table.HasValue(UGS[CURRENT_USERGROUP].string_tools, "all") then cb:SetChecked(true) end
		function cb:OnChange(bVal)
			...
			for j, cat2 in pairs(cat.Items) do
				for k, too in pairs(cat2) do
					if type(too) == "table" then
						...
						if type(UGS[CURRENT_USERGROUP].string_tools) == "string" then UGS[CURRENT_USERGROUP].string_tools = string.Explode(",", UGS[CURRENT_USERGROUP].string_tools) end
						if table.HasValue(UGS[CURRENT_USERGROUP].string_tools, too.ItemName) then cb2:SetChecked(true) end
		...
		for i, v in pairs(properties) do
			...
			if table.HasValue(UGS[CURRENT_USERGROUP].string_tools, v) then cb3:SetChecked(true) end
	...
	function YRPUpdateAmmoAmountUG()
		...
		for i, v in pairs(ammos) do
			if tonumber(v) > 0 then table.insert(tab, i .. ":" .. v) end
		end
...
function YRPAddUG(tbl)
	...
	function _ug:Paint(pw, ph)
		...
		if not strEmpty(UGS[self.uid].string_displayname) then text = text .. " " .. "( " .. tostring(UGS[self.uid].string_displayname) .. " )" end
		...
		if UGS[tonumber(tbl.uniqueID)].string_icon and strEmpty(UGS[tonumber(tbl.uniqueID)].string_icon) then draw.RoundedBox(0, YRP:ctr(8) + YRP:ctr(40) + YRP:ctr(8), YRP:ctr(4), ph - YRP:ctr(8), ph - YRP:ctr(8), Color(255, 255, 255, 255)) end
...
net.Receive("nws_yrp_updateUsergroupsList", function()
	...
	_icon.br = YRP:ctr(8)
	local ug = {}
	if net.ReadBool() then YRPClearUGList() end
	...
	YRPAddToUGList(ug)
	if net.ReadBool() then UpdateUsergroupsList() end
end)
