...
function BuildTable(tab, parent, name, tabx)
	...
	function header:Paint(pw, ph)
		...
		if name then draw.SimpleText(name .. ":", "Y_14_500", tabx + YRP:ctr(10), ph / 2, Color(255, 255, 255, 255), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER) end
	end

	if parent then parent:AddItem(header) end
	if tab then
		for i, v in pairs(tab) do
			...
			elseif type(v) == "boolean" then
				...
				if parent then parent:AddItem(pnl) end
			end
		end
	end
end
