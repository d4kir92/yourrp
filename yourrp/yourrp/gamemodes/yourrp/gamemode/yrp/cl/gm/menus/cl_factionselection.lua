...
function CreateFactionSelectionContent()
	...
	if LocalPlayer().cc then
		...
		function back:Paint(pw, ph)
			if LocalPlayer():GetYRPInt("char_count", 0) > 0 then hook.Run("YButtonRPaint", self, pw, ph) end
		end
	...
	net.Receive("nws_yrp_factionselection_getfactions", function(len)
		...
		if YRPPanelAlive(lis, "lis") then
			...
			if table.Count(ftab) > 1 then
				...
				local h = YRP:ctr(500)
				if table.Count(ftab) > 4 then lis:SetWide(nw + sbar:GetWide()) end
				-- Recalculate Height
				...
				for i, fac in SortedPairsByMemberValue(ftab, "int_position") do
					...
					if fac.uniqueID ~= -1 then
						...
						function join:Paint(pw, ph)
							if self:IsHovered() then draw.RoundedBox(YRP:ctr(10), 0, 0, pw, ph, Color(255, 255, 255, 1)) end
						end
