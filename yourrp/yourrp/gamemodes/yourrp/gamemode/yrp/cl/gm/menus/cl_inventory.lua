...
function YRPCloseInventory()
	inv.open = false
	if IsValid(YRPDropItem) then YRPDropItem:Hide() end
	if inv.win ~= nil then
		surface.PlaySound("ambient/materials/wood_creak4.wav")
		if inv.env then inv.env:Remove() end
		if inv.envstor then inv.envstor:Remove() end
		if inv.env2 then inv.env2:Remove() end
		if inv.envstor2 then inv.envstor2:Remove() end
		inv.win:Remove()
		inv.win = nil
	end
end
...
function YRPOpenInventory(target)
	if IsInventorySystemEnabled() and YRPIsNoMenuOpen() then
		...
		if IsValid(YRPDropItem) then YRPDropItem:Show() end
...
net.Receive("nws_yrp_open_storage", function(len)
	...
	if YRPPanelAlive(YRPInventory(), "YRPInventory() 2") then
		...
		if inv and inv.env2 then inv.env2:Remove() end
		...
		function inv.env2:Paint(pw, ph)
			if not YRPInventory() then self:Remove() end
			draw.RoundedBox(0, 0, 0, pw, ph, YRPInterfaceValue("YFrame", "NC"))
