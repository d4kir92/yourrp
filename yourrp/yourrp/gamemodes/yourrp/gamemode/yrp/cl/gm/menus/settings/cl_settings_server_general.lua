...
function CreateTextBoxLine(dpanellist, text, lstr, netstr)
	...
	if text then textbox:SetText(text) end
...
function CreateComboBoxLine(dpanellist, text, lstr, netstr, default, choices)
	...
	for id, v in pairs(choices) do
		...
		if v == default then selected = true end
...
function AddToTabRecursive(tab, folder, path, wildcard)
	...
	if files then
		for k, v in pairs(files) do
			...
			if not table.HasValue(tab, folder .. v) then table.insert(tab, folder .. v) end
		end
	end
...
net.Receive("nws_yrp_connect_Settings_General", function(len)
	...
	if YRPPanelAlive(PARENT) then
		...
		CreateCheckBoxLineTab(GAMEMODE_SYSTEMS:GetContent(), GEN.bool_inventory_system, "LID_inventorysystem", "nws_yrp_update_bool_inventory_system")
		CreateCheckBoxLineTab(GAMEMODE_SYSTEMS:GetContent(), GEN.bool_level_system, "LID_levelsystem", "nws_yrp_update_bool_level_system")
		CreateCheckBoxLineTab(GAMEMODE_SYSTEMS:GetContent(), GEN.bool_weapon_system, "LID_weaponsystem", "nws_yrp_update_bool_weapon_system")
		...
		CreateCheckBoxLineTab(GAMEMODE_SYSTEMS:GetContent(), GEN.bool_building_system, "LID_buildingsystem_3d", "nws_yrp_update_bool_building_system_3d")
		CreateCheckBoxLineTab(GAMEMODE_SYSTEMS:GetContent(), GEN.bool_allbuildingsunlocked, "LID_allbuildingsunlocked", "nws_yrp_update_bool_allbuildingsunlocked")
		CreateCheckBoxLineTab(GAMEMODE_SYSTEMS:GetContent(), GEN.bool_securitylevel_system, "LID_securitylevelsystem", "nws_yrp_update_bool_securitylevel_system")
		CreateHRLine(GAMEMODE_SYSTEMS:GetContent())
		...
		function idcard_change:DoClick()
			...
			for i, ele in pairs(elements) do
				...
				if not string.find(ele, "box", 1, true) then name = YRP:trans("LID_" .. ele) end
				...
				function e:Paint(pw, ph)
					...
					self.inbg = false
					if GetGlobalYRPInt("int_" .. ele .. "_x", 0) < GetGlobalYRPInt("int_" .. "background" .. "_w", 0) and GetGlobalYRPInt("int_" .. ele .. "_y", 0) < GetGlobalYRPInt("int_" .. "background" .. "_h", 0) then self.inbg = true end
					...
					if self.toggle ~= nil then self.toggle:SetVisible(visible) end
					if self.setting ~= nil then self.setting:SetVisible(visible) end
					local a1 = 1
					local a2 = 2
					local a3 = 100
					if visible then
					...
					if not string.find(ele, "logo", 1, true) or GetGlobalYRPBool("bool_" .. ele .. "_visible", false) == false then
						...
						if GetGlobalYRPBool("bool_" .. ele .. "_visible", false) == true then bgcolor = Color(0, 255, 0, 1) end
						draw.RoundedBox(0, 0, 0, pw, ph, bgcolor)
					end

					--end
					if self:IsDraggable() or e.draggable then draw.RoundedBox(0, 0, 0, pw, self:GetHeaderHeight(), Color(60, 255, 60, a2)) end
					...
					if wrongsize then self:SetSize(w, h) end
					-- POSITION
					...
					if wrongpos then self:SetPos(x, y) end
				...
				function e.setting:DoClick()
					...
					for id, v in pairs(cho) do
						local selected = false
						if id == GetGlobalYRPInt("int_" .. ele .. "_colortype", 0) then selected = true end
