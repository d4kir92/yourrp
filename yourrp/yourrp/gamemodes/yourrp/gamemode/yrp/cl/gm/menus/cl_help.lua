...
net.Receive("nws_yrp_getsitehelp", function(len)
	if YRPPanelAlive(HELPMENU.mainmenu.site) then
		...
		if GetGlobalYRPBool("bool_appearance_system", false) then AddKeybind(keybinds, YRPGetKeybindName("menu_appearance"), "LID_appearance", "face", "bool_appearance_system") end
		AddKeybind(keybinds, YRPGetKeybindName("menu_emotes"), "LID_emotes", "smile")
		if GetGlobalYRPBool("bool_laws_system", false) then AddKeybind(keybinds, YRPGetKeybindName("menu_laws"), "LID_laws", "list") end
	...
	if YRPPanelAlive(HELPMENU.mainmenu.site) then
		...
		for i, pl in pairs(staff) do
			...
			function tmp.steam:DoClick()
				if pl.ShowProfile then pl:ShowProfile() end
			end
...
net.Receive("nws_yrp_getsiteyourrptranslations", function(len)
	if YRPPanelAlive(HELPMENU.mainmenu.site) then
		...
		for sho, language in SortedPairs(YRP:GetAllLanguages()) do
			...
			if width > _longestProgressText then _longestProgressText = width end
		end
		...
		for sho, language in SortedPairs(YRP:GetAllLanguages()) do
			...
			function lan:Paint(pw, ph)
				...
				if language.percentage ~= nil then
					...
					if colper < 120 then colper = 120 end
					self.textcol = Color(255 - colper, colper, 0)
				end

				if language.author == "" then self.textcol = Color(255, 255, 0) end
				hook.Run("YButtonPaint", self, pw, ph)
