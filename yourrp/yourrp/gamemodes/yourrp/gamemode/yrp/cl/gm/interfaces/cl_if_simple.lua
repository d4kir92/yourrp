...
hook.Add("YFramePaint", "YFrame_Material", function(self, pw, ph, tab)
	...
	if YRPGetInterfaceDesign() == "Material" then
		...
		if self.rad then r = self.rad end
		...
		if self.GetTitle ~= nil then draw.SimpleText(YRP:trans(self:GetTitle()), "Y_18_500", hh / 2, hh / 2, YRPInterfaceValue("YFrame", "HT"), TEXT_ALIGN_LEFT, TEXT_ALIGN_CENTER) end
		return true
	end
end)
...
hook.Add("YButtonPaint", "YButton_Material", function(self, pw, ph, tab)
	...
	if YRPGetInterfaceDesign() == "Material" then
		...
		elseif self:IsHovered() then
			...
			tcolor.g = 255
			tcolor.b = 255
			if tab.force then color = Color(255, 255, 255, 0) end
			self.hovering = false
			self.clicked = false
		end
...
hook.Add("YLabelPaint", "YLabel_Material", function(self, pw, ph, tab)
	...
	if YRPGetInterfaceDesign() == "Material" then
		...
		local tx = pw / 2
		if ax == 0 then tx = YRP:ctr(20) end
		local ty = ph / 2
		if ay == 3 then ty = YRP:ctr(20) end
