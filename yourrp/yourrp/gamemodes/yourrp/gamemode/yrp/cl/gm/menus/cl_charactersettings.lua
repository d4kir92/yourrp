...
function YRPCreateCharacterSettingsContent()
	...
	net.Receive( "yrp_char_getrole", function(len)
		...
		-- TOP
		local descheader = YRPCreateD( "YLabel", win, YRP.ctr( config.w - 2 * 20), YRP.ctr( config.hh), YRP.ctr(20), YRP.ctr(20) )
			...
			if string.len(sname) <= rol.int_namelength and string.len(sname) > 0 or rol.int_namelength == 0 then
				...
				net.Receive( "YRPCreateCharacter", function(len)
					...
					if success then
						...
						local win = YRPCreateD( "YFrame", nil, 400, 200, 0, 0)
						...
						win.warning = YRPCreateD( "YLabel", win, 380, 200 - win:GetHeaderHeight() - 20, 10, win:GetHeaderHeight() + 10)
						if invalid then
