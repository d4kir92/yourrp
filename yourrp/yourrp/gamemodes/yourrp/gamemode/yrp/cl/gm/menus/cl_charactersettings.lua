...
function YRPCreateCharacterSettingsContent()
	...
	net.Receive("nws_yrp_char_getrole", function(len)
		...
		if pms.models[tonumber(LocalPlayer().charcreate_rpmid)] ~= nil then pms:SetModel(pms.models[tonumber(LocalPlayer().charcreate_rpmid)]) end
		...
		function confirm:DoClick()
			...
			if string.len(sname) <= rol.int_namelength and string.len(sname) > 0 or rol.int_namelength == 0 then
				...
				if not strEmpty(LocalPlayer().charcreate_namelast) then name = name .. " " .. LocalPlayer().charcreate_namelast end
				...
				net.Receive("nws_yrp_create_own_character", function(nlen)
					local success = net.ReadBool()
					if success then
						if YRPPanelAlive(CharacterMenu, "CharacterMenu") then CharacterMenu:Remove() end
		...
		if rol.int_namelength > 0 then
			...
			name:SetText("")
			if name.SetPlaceholderText then name:SetPlaceholderText(YRP:trans("LID_enteryourfirstname")) end
			...
			surname:SetText("")
			if surname.SetPlaceholderText then surname:SetPlaceholderText(YRP:trans("LID_enteryoursurname")) end
