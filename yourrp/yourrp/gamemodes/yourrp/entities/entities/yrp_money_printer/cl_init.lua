...
net.Receive("nws_yrp_getMoneyPrintMenu", function(len)
	...
	if upgradeframe == nil then
		...
		--Printer
		if not GetGlobalYRPBool("bool_money_printer_spawn_money", false) then moneyPrinterButton(mp, upgradeframe, YRP:ctr(800 - 2 * 20), YRP:ctr(60), YRP:ctr(20), YRP:ctr(60 + 220), "storage", "nws_yrp_upgradeStorage", YRP:trans("LID_storage"), YRP:trans("LID_upgrade"), YRP:trans("LID_max")) end
		--Fuel
		...
		function workingB:Paint(pw, ph)
			...
			if mp:GetYRPBool("working") then working = YRP:trans("LID_on") end
			if self:IsHovered() then
				draw.RoundedBox(YRP:ctr(10), 0, 0, pw, ph, Color(255, 255, 0, 200))
...
function ENT:Draw()
	...
	if dist < 2800 then self:DrawModel() end
end
