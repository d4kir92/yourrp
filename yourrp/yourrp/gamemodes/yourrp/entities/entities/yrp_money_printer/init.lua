...
net.Receive("nws_yrp_fuelUp", function(len, ply)
	...
	if IsValid(mp) and mp:GetClass() == "yrp_money_printer" and mp:GetOwner() == ply then
		...
		if ply:canAfford(cost) and mp:GetYRPInt("fuel", 0) < mp:GetYRPInt("fuelMax", 0) then
			...
			if mp:GetYRPInt("fuel", 0) > mp:GetYRPInt("fuelMax", 0) then mp:SetYRPInt("fuel", mp:GetYRPInt("fuelMax", 0)) end
		end
	end
end)
...
net.Receive("nws_yrp_repairMP", function(len, ply)
	...
	if IsValid(mp) and mp:GetClass() == "yrp_money_printer" and mp:GetOwner() == ply then
		...
		if ply:canAfford(cost) and mp:GetYRPInt("hp") < mp:GetYRPInt("hpMax") then
			...
			if mp:GetYRPInt("hp") > mp:GetYRPInt("hpMax") then mp:SetYRPInt("hp", mp:GetYRPInt("hpMax")) end
		end
	end
end)
...
net.Receive("nws_yrp_startMoneyPrinter", function(len, ply)
	...
	if IsValid(mp) and mp:GetClass() == "yrp_money_printer" and mp:GetOwner() == ply then
		...
		elseif not mp:GetYRPBool("working") then
			if mp:GetYRPInt("fuel", 0) > 0 then mp:SetYRPBool("working", true) end
		end
	end
end)
...
function ENT:OnTakeDamage(dmginfo)
	if not self.m_bApplyingDamage then
		...
		if self:GetYRPInt("hp", 0) <= 0 then self:Destroy() end
...
function ENT:Think()
	...
	if self:GetYRPBool("working") then
		...
		if self:GetYRPInt("cooler", 0) > self:GetYRPInt("cpu", 0) then
			temp = temp - 1
			if temp < 34.0 and heated then temp = 34.0 end
		elseif self:GetYRPInt("cooler", 0) + 1 < self:GetYRPInt("cpu", 0) then
			...
			if temp > 34.0 then heated = true end
		end
	else
		temp = temp - 0.3
		heated = false
	...
	if temp < 0.0 then temp = 0.0 end
	self:SetYRPFloat("temp", temp)
	-- To much heat, explode
	if self:GetYRPFloat("temp", 0.0) > self:GetYRPFloat("tempMax", 90.0) then self:Destroy() end
	if self:GetYRPInt("money") ~= nil then
		if self:GetYRPInt("fuel", 0) > 0 and self:GetYRPBool("working") then
			...
			if self:GetYRPInt("fuel", 0) < 0 then self:SetYRPInt("fuel", 0) end
			self:SetYRPInt("money", self:GetYRPInt("money") + (30 * self:GetYRPInt("printer")))
			if self:GetYRPInt("money") > self:GetYRPInt("moneyMax") then self:SetYRPInt("money", self:GetYRPInt("moneyMax")) end
